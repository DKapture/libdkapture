...
DKapture::DataType string_to_datatype(const std::string& node_name) {
	if (node_name == "stat") return DKapture::PROC_PID_STAT;
	if (node_name == "io") return DKapture::PROC_PID_IO;
	if (node_name == "traffic") return DKapture::PROC_PID_traffic;
	if (node_name == "statm") return DKapture::PROC_PID_STATM;
	if (node_name == "schedstat") return DKapture::PROC_PID_SCHEDSTAT;
	if (node_name == "fd") return DKapture::PROC_PID_FD;
	if (node_name == "status") return DKapture::PROC_PID_STATUS;
	if (node_name == "net") return DKapture::PROC_PID_NET;
	if (node_name == "cmdline") return DKapture::PROC_PID_CMDLINE;
	if (node_name == "env") return DKapture::PROC_PID_ENV;
	if (node_name == "cwd") return DKapture::PROC_PID_CWD;
	if (node_name == "root") return DKapture::PROC_PID_ROOT;
...
static int print_task_sock(void *ctx, const void *data, size_t data_sz)
{
	...
	if (sk->family == AF_INET)
	{
		...
		printf("%10d %16s %10lu %3d %5u %5u %5u %s:%u %s:%u\n",
			   hdr->pid, hdr->comm, sk->ino, sk->fd,
			   sk->family, sk->type, sk->state,
			   lip, ntohs(sk->lport),
			   rip, ntohs(sk->rport));
	}
	else if (sk->family == AF_INET6)
	{
		...
		printf("%10d %16s %10lu %3d %5u %5u %5u "
			   ...
			   hdr->pid, hdr->comm, sk->ino, sk->fd,
			   sk->family, sk->type, sk->state,
			   lipv6, ntohs(sk->lport),
			   ripv6, ntohs(sk->rport));
	}
	else
	{
		printf("%10d %16s %10lu %3d %5u %5u %5u\n",
	...
	while (data_sz > (int)sizeof(DKapture::DataHdr))
	{
		...
		case DKapture::PROC_PID_STAT:
		{
			...
			printf("pid: %d tgid: %d comm: %s nice: %d pgid: %d utime: %lu stime: %lu\n",
				data->pid, data->tgid, data->comm, stat->nice, stat->pgid, stat->utime, stat->stime);
		}
		break;
		case DKapture::PROC_PID_IO:
		{
			...
			printf("pid: %d tgid: %d comm: %s rd: %lu wr: %lu\n",
				data->pid, data->tgid, data->comm, io->rchar, io->wchar);
		}
		break;
		case DKapture::PROC_PID_traffic:
		{
			...
			printf("pid: %d tgid: %d comm: %s rbytes: %lu wbytes: %lu\n",
				data->pid, data->tgid, data->comm, traffic->rbytes, traffic->wbytes);
		}
		break;
		case DKapture::PROC_PID_STATM:
		{
			...
			printf("pid: %8d comm: %15s vsize: %10d resident: %10d shared: %10d\n",
				data->pid, data->comm, statm->size, statm->resident, statm->shared);
		}
		break;
		...
		case DKapture::PROC_PID_FD:
		{
			...
			printf("pid: %d tgid: %d comm: %s fd: %d ino: %lu mode: %lu dev: %u\n",
				   ...
				   fd->fd, fd->inode, fd->i_mode, fd->dev);
		}
		break;
...
static const char *gen_mount_call(const struct mount_args *e)
{
	snprintf(g_call_buf, sizeof(g_call_buf),
			 "mount(\"%s\", \"%s\", \"%s\", %d, \"%s\") = `%d",
			 e->source, e->target, e->filesystemtype, e->flags,
			 e->data, e->ret);
	return g_call_buf;
}
...
{
	snprintf(g_call_buf, sizeof(g_call_buf),
			 "fsopen(\"%s\", %u) = %d", e->fsname, e->flags, e->ret);
	return g_call_buf;
}
...
static const char *gen_fsconfig_call(const struct fsconfig_args *e)
{
	snprintf(g_call_buf, sizeof(g_call_buf),
			 ...
			 e->fd, e->cmd, e->key, e->value, e->aux, e->ret);
	return g_call_buf;
}
...
static const char *gen_fsmovemount_call(const struct move_mount_args *e)
{
	snprintf(g_call_buf, sizeof(g_call_buf),
			 ...
			 e->from_dfd, e->from_pathname, e->to_dfd,
			 e->to_pathname, e->flags, e->ret);
	return g_call_buf;
}
...
static const char *gen_open_tree_call(const struct open_tree_args *e)
{
	snprintf(g_call_buf, sizeof(g_call_buf),
			 "open_tree(%d, \"%s\", %u) = %d",
			 e->dfd, e->filename, e->flags, e->ret);
	return g_call_buf;
}
...
static int print_mount_info(void *ctx, const void *data, size_t len)
{
	...
	case sizeof(mount_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_mount_call(e));
		break;
	}
	case sizeof(umount_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_umount_call(e));
		break;
	}
	case sizeof(fsopen_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_fsopen_call(e));
		break;
	}
	case sizeof(fsconfig_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_fsconfig_call(e));
		break;
	}
	case sizeof(fsmount_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_fsmount_call(e));
		break;
	}
	case sizeof(move_mount_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_fsmovemount_call(e));
		break;
	}
	case sizeof(fspick_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_fspick_call(e));
		break;
	}
	case sizeof(mount_setattr_args):
	{
		const struct mount_setattr_args *e = (const struct mount_setattr_args *)data;
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_mount_setattr_call(e));
		break;
	}
	case sizeof(open_tree_args):
	{
		...
		printf("%-16s %-7d %-7d %-11u %s\n",
			   e->comm, e->pid, e->tid, e->mnt_ns, gen_open_tree_call(e));
		break;
	}
	default:
...
static int print_irq_info(void *ctx, const void *data, size_t data_sz)
{
	...
	if (e->type == IRQ)
	{
		printf("[IRQ] pid=%d tid=%d comm=%s irq=%d name=%s delta=%lluns ret=%d\n",
			   e->pid, e->tid, e->comm, e->vec_nr, e->name, e->delta, e->ret);
	}
	else if (e->type == SOFT_IRQ)
	{
		printf("[SOFTIRQ] pid=%d tid=%d comm=%s vec=%s delta=%lluns ret=%d\n",
			   e->pid, e->tid, e->comm, vec_names[e->vec_nr], e->delta, e->ret);
	}
	return 0;
}
...
void run_procfs_read(DKapture* dk) {
	...
	if (!g_options.procfs_node.empty()) {
		// 如果指定了特定节点，只读取该节点
		DKapture::DataType specified_type = string_to_datatype(g_options.procfs_node);
...
void run_memory_scan(DKapture* dk) {
	...
	dk->kmemleak_scan_start(g_options.memory_scan_pid, print_call_stack, nullptr);
