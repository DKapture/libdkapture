...
static __inline void send_event_enhanced(__u32 sip, __u32 dip, __u32 sport, __u32 dport,
                                        ...
                                        struct packet_tuple *tuple) 
{
    ...
    if ((rule->match_mask & 1) && rule->target_ip != 0 && rule->target_ip != check_ip) 
    {
        DEBUG(DEBUG_ON, "IP mismatch: target=%u, actual=%u", rule->target_ip, check_ip);
        return false;
    }
    
    if ((rule->match_mask & 2) && rule->target_port != 0 && rule->target_port != check_port) 
    {
        DEBUG(DEBUG_ON, "Port mismatch");
        return false;
    }
    
    if ((rule->match_mask & 4) && rule->target_protocol != 0 && rule->target_protocol != tuple->protocol) 
    {
                                          ...
                                          struct packet_tuple *tuple)
{
    ...
    DEBUG(DEBUG_ON, "Basic IP: src=%u, dst=%u, protocol=%u", 
               tuple->src_ip, tuple->dst_ip, tuple->protocol);
...
static __inline void update_stats_generic(void *map, 
                                         const void *key, 
                                         __u32 packet_len, 
                                         bool dropped) 
{
...
static __inline void update_port_stats(__u16 port, __u32 packet_len, bool dropped) 
{
    update_stats_generic(&port_stats_map, &port, packet_len, dropped);
}

static __inline void update_all_stats(struct packet_tuple *tuple, __u32 packet_len, bool dropped) 
{
                                                             ...
                                                             int *error_code) 
{
    ...
    {
        DEBUG(DEBUG_ON, "Invalid params: time_scale=%u, rate_bps=%llu", time_scale, rate_bps);
        *error_code = 0;
        return NULL;
    }
    ...
    if (!b) 
    {
        ...
        int update_result = bpf_map_update_elem(&buckets, &bucket_key, &init, BPF_ANY);
        if (update_result != 0) 
        {
                                           ...
                                           __u32 time_scale) 
{
    ...
    DEBUG(DEBUG_ON, "delta_ns=%llu, tokens_to_add=%llu, max_bucket=%llu, current_tokens=%llu",
...
static __inline struct token_bucket_result apply_rate_limiting_safe(__u64 bucket_key,
                                                                  __u64 rate_bps,
                                                                  __u32 time_scale,
                                                                  __u32 packet_size) 
{
    DEBUG(DEBUG_ON, "apply_rate_limiting_safe: bucket_key=%llu, packet_size=%u", bucket_key, packet_size);
    ...
    int bucket_error = 0;
    struct rate_bucket *b = get_or_create_bucket_safe(bucket_key, rate_bps, time_scale, &bucket_error);
    if (!b || bucket_error != 1) 
    {
    ...
    DEBUG(DEBUG_ON, "Current tokens: %llu, packet_size: %u", b->tokens, packet_size);
    ...
    else 
    {
        ...
        DEBUG(DEBUG_ON, "Sufficient tokens, will pass, remaining: %llu", b->tokens);
    }
...
static int netfilter_handle(struct bpf_nf_ctx *ctx)
{
    ...
    if (!rule) 
    {
        ...
        send_event_enhanced(tuple.src_ip, tuple.dst_ip, tuple.src_port, tuple.dst_port, 
                           packet_len, 0, 1, 0, EVENT_PACKET_PASS);
        return NF_ACCEPT;
    }
    ...
    if (rule->rule_type == 1) 
    {
        ...
        send_event_enhanced(tuple.src_ip, tuple.dst_ip, tuple.src_port, tuple.dst_port, 
                           0, packet_len, 0, 1, EVENT_PACKET_DROP);
        return NF_DROP;
    }
    ...
    {
        send_event_enhanced(tuple.src_ip, tuple.dst_ip, tuple.src_port, tuple.dst_port, 
                           packet_len, 0, 1, 0, EVENT_STATS_UPDATE);
        return NF_ACCEPT;
    }
    ...
    if (rate_result.should_drop) 
    {
        ...
        update_all_stats(&tuple, packet_len, true);
        send_event_enhanced(tuple.src_ip, tuple.dst_ip, tuple.src_port, tuple.dst_port, 
                           0, packet_len, 0, 1, EVENT_RATE_LIMIT);
        return NF_DROP;
    }
