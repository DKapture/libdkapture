...
void handle_send_drop_event(const struct event_t *e)
{
	...
	{
		pr_warn(
			"[丢包] PID=%u (%s) 丢包: %u bytes (%u packets)",
			e->proc.pid,
			e->proc.comm,
			e->bytes_dropped,
			e->packets_dropped
	...
	{
		pr_info(
			"[发送] PID=%u (%s) 发送: %u bytes (%u packets)",
			e->proc.pid,
			e->proc.comm,
			e->bytes_sent,
			e->packets_sent
...
void handle_network_tuple_event(const struct event_t *e)
{
	...
	pr_info(
		"[网络三元组] PID=%u (%s) IP=%s 端口=%u 协议=%s 数据包大小=%u bytes",
		e->proc.pid,
		e->proc.comm,
		inet_ntoa(*(struct in_addr *)&e->ip),
		ntohs(e->port),
...
static int handle_traffic_event(void *ctx, void *data, size_t data_sz)
{
	...
	case 3: // IP_AND_PORT
		pr_info(
			"[IP和端口] PID=%u (%s) IP=%s 端口=%u 时间刻度=%u秒",
			e->proc.pid,
			e->proc.comm,
			inet_ntoa(*(struct in_addr *)&e->ip),
			ntohs(e->port),
...
{
	struct bpf_map *map =
		bpf_object__find_map_by_name(skel->obj, "process_rules");
	if (!map)
	{
...
{
	struct bpf_map *map =
		bpf_object__find_map_by_name(skel->obj, "local_ip_map");
	if (!map)
	{
...
int main(int argc, char **argv)
{
	...
	recvmsg_kprobe_link =
		bpf_program__attach(skel->progs.security_socket_recvmsg);
	...
	connect_kprobe_link =
		bpf_program__attach(skel->progs.security_socket_connect);
	...
	// Attach connect kprobe
	listen_kprobe_link =
		bpf_program__attach(skel->progs.security_socket_listen);
