 ...
 */
static __inline struct rate_bucket* get_or_create_bucket(__u32 bucket_key, __u64 rate_bps, __u32 time_scale) 
{
 ...
 * Update token bucket
 */
static __inline void update_token_bucket(struct rate_bucket *b, __u64 rate_bps, __u32 time_scale) 
{
    __u64 now = now_ns();
 ...
 */
static __inline bool check_and_consume_tokens(struct rate_bucket *b, __u32 packet_size) 
{
 ...
 */
static __inline int apply_basic_rate_limiting(__u32 bucket_key, __u64 rate_bps, __u32 time_scale, __u32 packet_size) 
{
    struct rate_bucket *b = get_or_create_bucket(bucket_key, rate_bps, time_scale);
    if (!b) 
    {
 ...
 */
static __inline int apply_advanced_rate_limiting(__u32 bucket_key, __u64 rate_bps, __u32 time_scale, __u32 packet_size) 
{
    struct rate_bucket *b = get_or_create_bucket(bucket_key, rate_bps, time_scale);
    if (!b) 
    {
 ...
 */
static __inline int apply_rate_limiting_policy(__u32 bucket_key, __u64 rate_bps, __u32 time_scale, __u32 packet_size, __u8 policy) 
{
    switch (policy) 
    {
        case 0: // Basic rate limiting
            return apply_basic_rate_limiting(bucket_key, rate_bps, time_scale, packet_size);
        case 1: // Advanced rate limiting with 2x burst tolerance
            return apply_advanced_rate_limiting(bucket_key, rate_bps, time_scale, packet_size);
 ...
 */
static __inline void report_traffic_event_with_params(const struct event_params_t *params) 
{
    struct event_t *e = bpf_ringbuf_reserve(&ringbuf, sizeof(*e), 0);
    if (!e) 
    {
...
static __inline __u32 get_packet_type_id(__u16 eth_type) 
{
    switch (eth_type) 
    {
        case 0x0800: return 1;  // IPv4
        case 0x0806: return 2;  // ARP
        case 0x86DD: return 3;  // IPv6
        case 0x8100: return 4;  // 802.1Q VLAN
        case 0x8847: return 5;  // MPLS
        case 0x8864: return 6;  // PPPoE
 ...
 */
static __inline void update_flow_rate_stats(__u32 packet_type_id, __u32 packet_size) 
{
    if (packet_type_id == 0) 
    {
        return;
    ...
    struct flow_rate_info *flow_info = bpf_map_lookup_elem(&flow_rate_stats, &packet_type_id);
    ...
    if (flow_info) 
    {
        if (now - flow_info->window_start_ns >= NSEC_PER_SEC) 
        {
            ...
            {
                flow_info->rate_bps = (flow_info->total_bytes * 8 * NSEC_PER_SEC) / (now - flow_info->window_start_ns);
        ...
        bpf_map_update_elem(&flow_rate_stats, &packet_type_id, flow_info, BPF_ANY);
    } 
    else 
    {
        ...
        bpf_map_update_elem(&flow_rate_stats, &packet_type_id, &new_flow, BPF_ANY);
    }
}
...
static int tc_handle(struct __sk_buff *ctx, int gress)
{
    ...
    int result = apply_rate_limiting_policy(bucket_key, rule->rate_bps, rule->time_scale, ctx->len, 0);
