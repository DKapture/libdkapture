...
{
    pr_debug("safe_str_to_int called with str='%s', min=%d, max=%d, context=%s", str, min_val, max_val, context.c_str());
    ...
    if constexpr (std::is_signed_v<T>) 
    {
        ...
        if (errno == ERANGE || val < static_cast<long>(min_val) || val > static_cast<long>(max_val)) 
        {
            ...
            pr_error("Value out of range in %s: %ld (min: %d, max: %d)", context.c_str(), val, min_val, max_val);
            return false;
        }
    ...
    else 
    {
        ...
        {
            pr_debug("Value out of range");
            pr_error("Value out of range in %s: %lu (max: %d)", context.c_str(), val, max_val);
            return false;
        }
        *result = static_cast<T>(val);
    }
...
static int handle_traffic_event(void *ctx, void *data, size_t data_sz)
{
    ...
    {
        pr_info("[DROP] %u bytes, %u packets", e->bytes_dropped, e->packets_dropped);
    }
    ...
    pr_info("    EtherType:  0x%x (%s)", e->eth_type, get_ethertype_description(e->eth_type).c_str());
    pr_info("    Packet Size: %u bytes", e->packet_size);
    
    // Display packet type statistics and flow rate
    pr_info("  Packet Type Statistics:");
    pr_info("    Type: %s (ID: %u)", get_packet_type_description(e->packet_type).c_str(), e->packet_type);
    pr_info("    Current Flow Rate: %s", format_flow_rate(e->type_rate_bps).c_str());
    pr_info("    Smooth Flow Rate: %s (EMA)", format_flow_rate(e->type_smooth_rate_bps).c_str());
...
void parse_args(int argc, char **argv)
{
    ...
    pr_info("Rate limit: %llu B/s (%.2f MB/s)", 
            rule.rate_bps, (rule.rate_bps / 1024.0 / 1024.0));
...
static bool setup_traffic_rules()
{
    ...
    pr_info("Error count during setup: %zu", error_handler_get_error_count(&global_error_handler));
...
static bool validate_configuration()
{
    ...
    {
        pr_error("Invalid time scale: %u (must be between 1 and 3600 seconds)", rule.time_scale);
        safe_string_destroy(interface_str);
        return false;
    }
...
static void display_statistics()
{
    ...
    pr_info("Max bucket capacity: %.2f MB", (rule.rate_bps * rule.time_scale / 1024.0 / 1024.0));
    pr_info("===================================");
...
static void display_packet_type_summary()
{
    ...
    pr_info("Timestamp: %ld", std::chrono::duration_cast<std::chrono::seconds>(
...
int main(int argc, char **argv)
{
    ...
    {
        pr_error("Failed to create TC hook (egress direction): %d (%s)", err, strerror(-err));
    ...
    {
        pr_error("Failed to create TC hook (ingress direction): %d (%s)", err, strerror(-err));
    ...
    if (err)
    {
        pr_error("Failed to attach TC program (egress direction): %d (%s)", err, strerror(-err));
    ...
    if (err)
    {
        pr_error("Failed to attach TC program (ingress direction): %d (%s)", err, strerror(-err));
    ...
    pr_info("Successfully attached TC program to interface %u (egress direction)", ifindex);
    pr_info("Successfully attached TC program to interface %u (ingress direction)", ifindex);
...
cleanup:
    ...
    pr_info("Final error count: %zu", error_handler_get_error_count(&global_error_handler));
