    ...
    struct dentry *dentry)
{
    ...
    if (rule->inode &&
        rule->inode != dentry->d_inode->i_ino)
    {
        return 0;
    }
             ...
             int ret)
{
    if (ret)
    {
        DEBUG(0, "rm-forbid %s early return for previous bpf-lsm programs",
              __func__);
        return ret;
    }
