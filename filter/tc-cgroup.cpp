...
void Usage(const char *arg0) {
    ...
    std::cout << "  -d, --direction <dir>  Match direction (egress=outgoing, ingress=incoming)" << std::endl;
    ...
    std::cout << std::endl;
    std::cout << "Note: This program will limit traffic for the entire cgroup, not individual processes" << std::endl;
}
...
void parse_args(int argc, char **argv) {
    ...
    std::cout << "Match direction: " << (rule.gress ? "EGRESS (outgoing)" : "INGRESS (incoming)") << std::endl;
...
static bool setup_cgroup_rules() {
    struct bpf_map *map = bpf_object__find_map_by_name(skel->obj, "cgroup_rules");
    if (!map) {
...
int main(int argc, char **argv) {
    ...
    if (!skel) {
        std::cerr << "Failed to open and load BPF skeleton: " << strerror(errno) << std::endl;
        return 1;
    }
    ...
    if (cgroup_fd < 0) {
        std::cerr << "Failed to open cgroup: " << cgroup_path << " (" << strerror(errno) << ")" << std::endl;
        err = -1;
        goto cleanup;
    }
    ...
    if (rule.gress == EGRESS) {
        err = bpf_prog_attach(bpf_program__fd(skel->progs.cgroup_skb_egress), cgroup_fd, 
                              BPF_CGROUP_INET_EGRESS, 0);
     } else {
        err = bpf_prog_attach(bpf_program__fd(skel->progs.cgroup_skb_ingress), cgroup_fd, 
                              BPF_CGROUP_INET_INGRESS, 0);
     }
    
    if (err) {
        std::cerr << "Failed to attach cgroup program: " << err << " (" << strerror(-err) << ")" << std::endl;
        goto cleanup;
    }
    
    std::cout << "Successfully attached cgroup program to " << cgroup_path << std::endl;
    std::cout << "Match direction: " << (rule.gress ? "EGRESS (outgoing)" : "INGRESS (incoming)") << std::endl;
    std::cout << "Rate limit: " << rule.rate_bps << " B/s" << std::endl;
    std::cout << "Note: Will limit traffic for all processes in the cgroup" << std::endl;
    ...
    // Detach eBPF program
    if (rule.gress == EGRESS) {
        err = bpf_prog_detach2(bpf_program__fd(skel->progs.cgroup_skb_egress), cgroup_fd, 
                               BPF_CGROUP_INET_EGRESS);
    } else {
        err = bpf_prog_detach2(bpf_program__fd(skel->progs.cgroup_skb_ingress), cgroup_fd, 
