...
static bool parse_ip_address(const char *str, uint32_t *result, const std::string& context = "");
static bool parse_protocol(const char *str, uint8_t *result, const std::string& context = "");
...
template<typename T>
static bool safe_str_to_int(const char *str, T *result, T min_val, T max_val, const std::string& context = "");
...
void parse_args(int argc, char **argv) 
{
    ...
    while ((opt = getopt_long(argc, argv, sopts.c_str(), lopts, &opt_idx)) > 0) 
    {
        ...
        case 'p': // Target port
            ...
            else if (!safe_str_to_int(optarg, &rule.target_port, static_cast<uint16_t>(1), static_cast<uint16_t>(65535))) 
            {
        ...
        case 'P': // Target protocol
            ...
            {
                pr_error("Invalid protocol '%s'. Supported: tcp, udp, any", optarg);
                exit(-1);
            }
            break;
        case 'r': // Rate limit
        ...
        case 'T': // Rule type
            ...
            {
                pr_error("Invalid rule type '%s'. Supported: rate, drop, log", optarg);
                exit(-1);
            }
            break;
    ...
    rule.match_mask = 0;
    if (rule.target_ip != 0) rule.match_mask |= 1;
    if (rule.target_port != 0) rule.match_mask |= 2;
    if (rule.target_protocol != 0) rule.match_mask |= 4;
    ...
    else 
    {
        ...
        if (rule.target_protocol != 0) 
        {
            ...
            if (rule.target_protocol == IPPROTO_TCP) pr_info(" (TCP)");
            else if (rule.target_protocol == IPPROTO_UDP) pr_info(" (UDP)");
        } 
        else 
        {
    ...
    pr_info("Match direction: %s", (rule.gress ? "EGRESS (destination)" : "INGRESS (source)"));
    if (rule.rule_type == RULE_TYPE_RATE_LIMIT) 
    {
        pr_info("Time scale: %u seconds (max bucket capacity: %.2f MB)", 
                rule.time_scale, (rule.rate_bps * rule.time_scale / 1024.0 / 1024.0));
    }
}
...
static uint64_t parse_bandwidth(const char *str, const std::string& context) 
{
    if (!str) 
    {
        pr_warn("Null bandwidth string pointer in %s, using default", context.c_str());
    ...
    pr_warn("Invalid bandwidth suffix in %s: %s, using default", context.c_str(), endptr);
    return DEFAULT_RATE_BPS;
}
...
static bool parse_protocol(const char *str, uint8_t *result, const std::string& context) 
{
    ...
    {
        if (!safe_str_to_int(str, result, static_cast<uint8_t>(0), static_cast<uint8_t>(255), context)) 
        {
            pr_error("Failed to parse protocol number in %s: %s", context.c_str(), str);
            return false;
        }
    }
    return true;
...
static std::string format_ip(uint32_t ip, const std::string& context) 
{
    try 
    {
        ...
        if (!result) 
        {
            pr_error("Failed to format IP address in %s: %u", context.c_str(), ip);
            return "0.0.0.0";
        }
    ...
    {
        pr_error("Exception in IP formatting in %s: %s", context.c_str(), e.what());
        return "0.0.0.0";
    }
}
...
T* safe_allocate(size_t count, const std::string& context = "") 
{
    try 
    {
        ...
        if (!ptr) 
        {
            pr_error("Failed to allocate memory for %zu elements in %s", count, context.c_str());
        }
        return ptr;
    } 
    catch (const std::bad_alloc& e) 
    {
        pr_error("Memory allocation failed in %s: %s", context.c_str(), e.what());
        return nullptr;
    }
}
...
void safe_deallocate(T* ptr, const std::string& context = "") 
{
    if (ptr) 
    {
        ...
        {
            pr_error("Memory deallocation failed in %s: %s", context.c_str(), e.what());
        }
    }
}
    ...
    switch (e->event_type) 
    {
        case 0:
            pr_info("Traffic: %s:%d -> %s:%d [PASS] %u bytes", 
                    src_ip.c_str(), e->sport, dst_ip.c_str(), e->dport, e->bytes_sent);
            break;
            
        case 1:
            pr_info("Traffic: %s:%d -> %s:%d [DROP] %u bytes", 
                    src_ip.c_str(), e->sport, dst_ip.c_str(), e->dport, e->bytes_dropped);
            break;
...
{
    SafeString* context_str = safe_string_create("setup_traffic_rules", "traffic control configuration");
    if (!safe_string_is_valid(context_str)) 
    {
        log_error(ErrorCode::MEMORY_ALLOCATION_FAILED, "Failed to create context string for traffic rules setup");
        return false;
    }
    ...
    if (!map) 
    {
        log_error(ErrorCode::BPF_OPERATION_FAILED, "Cannot find traffic_rules map");
    ...
    if (err) 
    {
        log_error(ErrorCode::BPF_OPERATION_FAILED, "Failed to set traffic control rules: " + std::to_string(err));
        pr_error("Failed to set traffic control rules: %d", err);
        safe_string_destroy(context_str);
        return false;
    }
    
    std::string context = safe_string_c_str(context_str) ? safe_string_c_str(context_str) : "unknown";
    std::string context_info = safe_string_context(context_str) ? safe_string_context(context_str) : "no context";
...
int main(int argc, char **argv) 
{
    ...
    if (!skel) 
    {
        log_error(ErrorCode::BPF_OPERATION_FAILED, "Failed to open and load BPF skeleton");
    ...
    {
        log_error(ErrorCode::TC_OPERATION_FAILED, "Failed to setup traffic control rules");
    ...
    {
        log_error(ErrorCode::NETWORK_OPERATION_FAILED, "Failed to attach netfilter ingress program");
        pr_error("Failed to attach netfilter ingress program: %s", strerror(errno));
        goto cleanup;
    }
    ...
    {
        log_error(ErrorCode::NETWORK_OPERATION_FAILED, "Failed to attach netfilter egress program");
        pr_error("Failed to attach netfilter egress program: %s", strerror(errno));
        close(netfilter_fd_ingress);
        goto cleanup;
    }
       
    pr_info("Successfully attached netfilter program (both directions)");
    pr_info("Match direction: %s", (rule.gress ? "EGRESS (destination IP:port)" : "INGRESS (source IP:port)"));
    
    rb = ring_buffer__new(bpf_map__fd(skel->maps.ringbuf), handle_traffic_event, nullptr, nullptr);
    if (!rb) 
    {
        log_error(ErrorCode::BPF_OPERATION_FAILED, "Failed to create ring buffer");
    ...
    while (running) 
    {
        ...
        if (err < 0) 
        {
            log_error(ErrorCode::BPF_OPERATION_FAILED, "Ring buffer polling error: " + std::to_string(err));
