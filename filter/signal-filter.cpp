...
static const SignalMapping signal_map[] = {
    {nullptr, 0}, {"SIGHUP", 1}, {"SIGINT", 2}, {"SIGQUIT", 3}, {"SIGILL", 4},
...
static const size_t signal_map_size = sizeof(signal_map) / sizeof(signal_map[0]);
...
static int handle_trace_event(void *ctx, void *data, size_t data_sz) {
    ...
    if (strnlen(e->sender_comm, sizeof(e->sender_comm)) >= sizeof(e->sender_comm) ||
        strnlen(e->target_comm, sizeof(e->target_comm)) >= sizeof(e->target_comm)) {
    ...
    std::cout << std::setw(8) << std::left << time_str << " "
              ...
              << std::setw(12) << std::left << e->sender_comm << "  "
              << std::setw(10) << std::right << e->target_pid << "           "
              << std::setw(12) << std::left << e->target_comm << "       "
              << std::setw(12) << std::left << signal_name << "   "
              << std::setw(12) << std::left << e->result << "   "
              << std::fixed << std::setprecision(2) << std::setw(8) << latency_us << " us" << std::endl;
...
static int handle_intercept_event(void *ctx, void *data, size_t data_sz) {
    ...
    // Validate string lengths to prevent buffer overflows
    if (strnlen(e->target_comm, sizeof(e->target_comm)) >= sizeof(e->target_comm)) {
        std::cerr << "Invalid string length in event data" << std::endl;
        return -1;
    }
...
int (*handle_array[EVENTNUMBER])(void *ctx, void *data, size_t data_sz) = {handle_trace_event, handle_intercept_event};
...
static void sig_handler(int sig) {
    std::cout << "\nReceived signal " << get_signal_name(sig) << ", exiting..." << std::endl;
    running = false;
}
...
void Usage(const char *arg0) {
    ...
    std::cout << "  Trace and intercept inter-process signal communication." << std::endl << std::endl;
    ...
    std::cout << std::endl;
    std::cout << "Rule Logic: All rules must be satisfied to intercept signals" << std::endl;
    std::cout << "Default Behavior: Allow all signals when no rules are set" << std::endl;
...
static bool safe_str_to_pid_t(const char *str, pid_t *result) {
    return safe_str_to_int(str, result, static_cast<pid_t>(1), static_cast<pid_t>(INT_MAX));
}

static bool safe_str_to_uid_t(const char *str, uid_t *result) {
    return safe_str_to_int(str, result, static_cast<uid_t>(0), static_cast<uid_t>(UINT_MAX));
}
...
void parse_args(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv, sopts.c_str(), lopts, &opt_idx)) > 0) {
        ...
        case 'P': // Sender process ID
            if (!safe_str_to_pid_t(optarg, &rule.sender_pid) || rule.sender_pid == 0) {
                std::cerr << "Error: Invalid sender PID '" << optarg << "'. PID must be positive" << std::endl;
                exit(-1);
            }
            break;
        case 'p': // Receiver process ID
            if (!safe_str_to_pid_t(optarg, &rule.recv_pid) || rule.recv_pid == 0) {
                std::cerr << "Error: Invalid receiver PID '" << optarg << "'. PID must be positive" << std::endl;
                exit(-1);
            }
            break;
        case 'U': // Sender user ID
            if (!safe_str_to_uid_t(optarg, &rule.sender_uid)) {
                std::cerr << "Error: Invalid sender UID '" << optarg << "'. UID must be non-negative" << std::endl;
                exit(-1);
            }
            break;
        case 'S': // Signal type
            ...
            if (rule.sig == -1) {
                uint32_t sig_num;
                if (!safe_str_to_uint32(optarg, &sig_num) || sig_num == 0 || sig_num > 31) {
                    std::cerr << "Error: Invalid signal '" << optarg << "'. ";
                    std::cerr << "Use signal name (e.g., SIGUSR1) or number (1-31)" << std::endl;
                    exit(-1);
                }
                rule.sig = sig_num;
            }
    ...
    std::cout << "\n=============== Filter Rules =================" << std::endl << std::endl;
    ...
    std::cout << "\tSignal type = " << rule.sig << " (" << get_signal_name(rule.sig) << ")" << std::endl;
    std::cout << "\n========================================" << std::endl << std::endl;
}
...
static bool setup_bpf_maps(struct signal_filter_bpf *skel) {
    ...
    err = bpf_map__update_elem(skel->maps.interception_mode, &key, sizeof(key), &interception_mode, sizeof(interception_mode), BPF_ANY);
    if (err) {
    ...
    err = bpf_map__update_elem(skel->maps.filter_rules, &key, sizeof(key), &rule, sizeof(rule), BPF_ANY);
    if (err) {
...
static bool setup_ring_buffer(struct signal_filter_bpf *skel, struct ring_buffer **rb) {
    *rb = ring_buffer__new(bpf_map__fd(skel->maps.ringbuf), handle_all_event, nullptr, nullptr);
    if (!*rb) {
...
static void print_startup_info() {
    ...
    std::cout << "  - Multiple filter criteria support (PID, UID, Signal)" << std::endl;
    std::cout << std::endl;
    std::cout << "SIGNAL TRACING:" << std::endl;
    std::cout << "TIME      SENDER           S-COMM             RCVER           R-COMM             SIGNAL             RESULT        LATENCY" << std::endl;
    std::cout << std::endl;
    std::cout << "SIGNAL INTERCEPTION:" << std::endl;
    std::cout << "TIME      TARGET_PID       TARGET_COMM        SIGNAL          ACTION" << std::endl;
    std::cout << std::endl;
}
...
static void cleanup_resources(struct ring_buffer *rb, struct signal_filter_bpf *skel) {
