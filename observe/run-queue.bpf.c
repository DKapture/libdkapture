...
static int rule_filter(struct task_struct *task)
{
    ...
    if (task->pid < rule->pid.min ||
        task->pid > rule->pid.max)
    {
        DEBUG(0, "filtered by pid: %d", task->pid);
        return 0;
    }

    if (task->tgid < rule->tgid.min ||
        task->tgid > rule->tgid.max)
    {
        DEBUG(0, "filtered by tgid: %d", task->tgid);
        return 0;
    }

    if (task->stime < rule->stime.min ||
        task->stime > rule->stime.max)
    {
        DEBUG(0, "filtered by stime: %lu", task->stime);
        return 0;
    }

    if (task->utime < rule->utime.min ||
        task->utime > rule->utime.max)
    {
    ...
    if (task->prio < rule->priority.min ||
        task->prio > rule->priority.max)
    {
