...
static char *resolve_kernel_symbol(uint64_t addr) {
    ...
    if (best_match) {
        if (best_addr == addr) {
            ...
            snprintf(symbol, sizeof(symbol), "%s+0x%" PRIx64, best_match, (uint64_t)(addr - best_addr));
        }
        free(best_match);
    } else {
        snprintf(symbol, sizeof(symbol), "0x%" PRIx64, (uint64_t)addr);
...
static char *format_timestamp(uint64_t ns) {
    ...
    if (env.timestamp) {
        ...
        snprintf(ts_str, sizeof(ts_str), "%02d:%02d:%02d.%06" PRIu64,
                tm->tm_hour, tm->tm_min, tm->tm_sec, (uint64_t)(nsec / 1000));
    } else {
        snprintf(ts_str, sizeof(ts_str), "%ld.%06" PRIu64, (long)sec, (uint64_t)(nsec / 1000));
    }
    ...
    switch (e->event_type) {
        ...
        case WQ_EVENT_START:
            ...
            if (e->delay_ns > 0) {
                ...
                if (stats.queue_stats.min_delay == 0 || e->delay_ns < stats.queue_stats.min_delay)
            ...
            break;
        case WQ_EVENT_END:
            if (e->delay_ns > 0) { // execution time
                stats.execute_stats.total_exec_time += e->delay_ns;
                if (stats.execute_stats.min_exec_time == 0 || e->delay_ns < stats.execute_stats.min_exec_time)
...
static void print_statistics() {
    ...
    if (stats.queue_stats.total_delay > 0) {
        printf("\nQueue Delays:\n");
        printf("  Average: %s\n", format_duration(stats.queue_stats.total_delay / stats.execute_stats.count));
    ...
    if (stats.execute_stats.total_exec_time > 0) {
        printf("\nExecution Times:\n");
        printf("  Average: %s\n", format_duration(stats.execute_stats.total_exec_time / stats.execute_stats.count));
...
static int setup_filters() {
    ...
    if (env.target_workqueue) {
        ...
        strncpy(cfg.target_workqueue, env.target_workqueue, WORKQUEUE_NAME_LEN - 1);
    }
    
    if (env.target_function) {
        cfg.filter_function = 1;
        strncpy(cfg.target_function, env.target_function, sizeof(cfg.target_function) - 1);
    }
...
static int parse_args(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv, "p:c:w:f:d:stjvh", long_options, NULL)) != -1) {
        switch (opt) {
        case 'p':
...
int main(int argc, char **argv) {
    ...
    if (!env.json_output && !env.statistics_mode) {
        printf("%-18s %-6s %-3s %-16s %-8s %-14s %-30s %-20s %-3s %12s\n",
               "TIME", "PID", "CPU", "COMM", "EVENT", "WORK", "FUNCTION", "WORKQUEUE", "REQ", "DELAY/EXEC");
    }
    ...
    while (!exiting) {
        ...
        int nfds = epoll_wait(epoll_fd, events, 2, 10); // 10ms timeout for faster response
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (events[i].data.fd == ring_fd) {
                ...
                // Signal received
                char buf[256];
                read(signal_pipe[0], buf, sizeof(buf)); // Drain the pipe
