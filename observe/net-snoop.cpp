...
static HelpMsg help_msg[] = {
    ...
    {"[mask]", "event mask: 1=queue, 2=start_xmit, 4=xmit, 8=receive (default: 15=all)\n"},
    ...
    while ((opt = getopt_long(argc, argv, sopts.c_str(), lopts, &opt_idx)) > 0) {
        ...
        case 'f':
            if (strcmp(optarg, "tcp") == 0) {
                ...
                fprintf(stderr, "Invalid protocol filter: %s. Must be tcp, udp, or icmp.\n", optarg);
                exit(-1);
            }
            break;
        case 'o':
...
static std::string format_tcp_info(const struct net_event *e)
{
    ...
    // 端口信息
    result += std::to_string(e->src_port) + " -> " + std::to_string(e->dst_port);
    ...
    if (e->tcp_flags != 0) {
        ...
        // 检查标志位是否在有效范围内
        if (e->tcp_flags & ~(TCP_FLAG_SYN | TCP_FLAG_ACK | TCP_FLAG_FIN | TCP_FLAG_RST)) {
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    printf("%-8s %-16s %-6d %-6d %-10s %-8s %-6u",
           time_buf, e->comm, e->pid, e->tid, 
           event_type_str(e->event_type), e->dev_name, e->len);
    
    if (e->event_type == 1) { // start_xmit 显示更多详细信息
        printf(" %-8s Q:%-3u GSO:%-4u", 
               protocol_str(e->protocol), e->queue_id, e->gso_size);
        if (e->vlan_tagged)
            printf(" VLAN:%04x", e->vlan_proto);
    } else if (e->event_type == 2) { // xmit 显示返回码和延迟
