...
static void print_event_alloc_dev(struct xhci_alloc_dev_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s info=0x%x info2=0x%x tt_info=0x%x state=0x%x\n",
           e->comm, e->pid, e->tid, "xhci_alloc_dev",
           e->info, e->info2, e->tt_info, e->state);
}
...
static void print_event_free_dev(struct xhci_free_dev_event_t *e)
{
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s info=0x%x info2=0x%x tt_info=0x%x state=0x%x\n",
           e->comm, e->pid, e->tid, "xhci_free_dev",
           e->info, e->info2, e->tt_info, e->state);
}
...
static void print_event_urb_enqueue(struct xhci_urb_enqueue_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s urb=0x%llx ep%d%s-%s slot=%d len=%d/%d stream=%u\n",
           e->comm, e->pid, e->tid, "xhci_urb_enqueue",
           e->urb, e->epnum, e->dir_in ? "in" : "out", get_usb_type_string(e->type),
...
static void print_event_urb_giveback(struct xhci_urb_giveback_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s urb=0x%llx ep%d%s-%s slot=%d len=%d/%d status=%d\n",
           e->comm, e->pid, e->tid, "xhci_urb_giveback",
           e->urb, e->epnum, e->dir_in ? "in" : "out", get_usb_type_string(e->type),
...
static void print_event_handle_event(struct xhci_handle_event_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s type=%u trb=0x%x,0x%x,0x%x,0x%x\n",
           e->comm, e->pid, e->tid, "xhci_handle_event",
           e->type, e->field0, e->field1, e->field2, e->field3);
}
...
static void print_event_handle_transfer(struct xhci_handle_transfer_event_t *e)
{
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s type=%u trb=0x%x,0x%x,0x%x,0x%x\n",
           e->comm, e->pid, e->tid, "xhci_handle_transfer",
           e->type, e->field0, e->field1, e->field2, e->field3);
}
...
static void print_event_queue_trb(struct xhci_queue_trb_event_t *e)
{
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s type=%u trb=0x%x,0x%x,0x%x,0x%x\n",
           e->comm, e->pid, e->tid, "xhci_queue_trb",
           e->type, e->field0, e->field1, e->field2, e->field3);
}
...
static void print_event_setup_device(struct xhci_setup_device_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s vdev=0x%llx devnum=%d state=%d speed=%d port=%d level=%d slot=%d\n",
           e->comm, e->pid, e->tid, "xhci_setup_device",
           e->vdev, e->devnum, e->state, e->speed, e->portnum, e->level, e->slot_id);
}
...
static void print_event_ring_alloc(struct xhci_ring_alloc_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s type=%u ring=0x%llx segs=%u stream=%u cycle=%u bounce=%u\n",
           e->comm, e->pid, e->tid, "xhci_ring_alloc",
           e->type, e->ring, e->num_segs, e->stream_id, e->cycle_state, e->bounce_buf_len);
}
...
static void print_event_add_endpoint(struct xhci_add_endpoint_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s info=0x%x info2=0x%x deq=0x%llx tx_info=0x%x\n",
           e->comm, e->pid, e->tid, "xhci_add_endpoint",
           e->info, e->info2, e->deq, e->tx_info);
}
...
static void print_event_address_ctx(struct xhci_address_ctx_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s ctx_64=%d type=%u dma=0x%llx va=0x%llx ep_num=%u\n",
           e->comm, e->pid, e->tid, "xhci_address_ctx",
           e->ctx_64, e->ctx_type, e->ctx_dma, e->ctx_va, e->ctx_ep_num);
}

static void print_event_alloc_virt_device(struct xhci_alloc_virt_device_event_t *e)
{
    if (should_filter_event(e->pid, e->tid, e->comm))
        return;
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s vdev=0x%llx devnum=%d state=%d speed=%d port=%d level=%d slot=%d\n",
           e->comm, e->pid, e->tid, "xhci_alloc_virt_device",
           e->vdev, e->devnum, e->state, e->speed, e->portnum, e->level, e->slot_id);
}
...
static void print_event_configure_endpoint(struct xhci_slot_ctx_event_t *e)
{
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s info=0x%x info2=0x%x tt_info=0x%x state=0x%x\n",
           e->comm, e->pid, e->tid, "xhci_configure_endpoint",
           e->info, e->info2, e->tt_info, e->state);
}
...
static void print_event_ring_free(struct xhci_ring_free_event_t *e)
{
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s type=%u ring=0x%llx segs=%u stream=%u cycle=%u bounce=%u\n",
           e->comm, e->pid, e->tid, "xhci_ring_free",
           e->type, e->ring, e->num_segs, e->stream_id, e->cycle_state, e->bounce_buf_len);
}
...
static void print_event_inc_deq(struct xhci_inc_deq_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s type=%u ring=0x%llx enq=0x%llx deq=0x%llx segs=%u stream=%u\n",
           e->comm, e->pid, e->tid, "xhci_inc_deq",
           e->type, e->ring, e->enq, e->deq, e->num_segs, e->stream_id);
}
...
static void print_event_inc_enq(struct xhci_inc_enq_event_t *e)
{
    ...
    print_timestamp();
    printf("%-16s %-7d %-7d %-20s type=%u ring=0x%llx enq=0x%llx deq=0x%llx segs=%u stream=%u\n",
           e->comm, e->pid, e->tid, "xhci_inc_enq",
           e->type, e->ring, e->enq, e->deq, e->num_segs, e->stream_id);
}
...
static void print_event_ring_ep_doorbell(struct xhci_ring_ep_doorbell_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s slot=%u doorbell=0x%x\n",
           e->comm, e->pid, e->tid, "xhci_ring_ep_doorbell",
           e->slot, e->doorbell);
}
...
static void print_event_urb_dequeue(struct xhci_urb_dequeue_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s urb=0x%llx ep%d%s-%s slot=%d len=%d/%d status=%d stream=%u\n",
           e->comm, e->pid, e->tid, "xhci_urb_dequeue",
           e->urb, e->epnum, e->dir_in ? "in" : "out", get_usb_type_string(e->type),
...
static void print_event_dbg_address(struct xhci_dbg_address_event_t *e)
{
    ...
    printf("%-16s %-7d %-7d %-20s msg=\"%s\"\n",
           e->comm, e->pid, e->tid, "xhci_dbg_address",
           e->msg);
}

// Generic print function for slot context events
static void print_event_slot_ctx(struct xhci_slot_ctx_event_t *e, const char *event_name)
{
    if (should_filter_event(e->pid, e->tid, e->comm))
        return;
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    case XHCI_ALLOC_VIRT_DEVICE:
        print_event_alloc_virt_device((struct xhci_alloc_virt_device_event_t *)data);
        break;
    ...
    case XHCI_RING_EP_DOORBELL:
        print_event_ring_ep_doorbell((struct xhci_ring_ep_doorbell_event_t *)data);
        break;
    ...
    case XHCI_SETUP_DEVICE_SLOT:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_setup_device_slot");
        break;
    case XHCI_HANDLE_CMD_ADDR_DEV:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_addr_dev");
        break;
    case XHCI_HANDLE_CMD_CONFIG_EP:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_config_ep");
        break;
    case XHCI_HANDLE_CMD_DISABLE_SLOT:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_disable_slot");
        break;
    case XHCI_HANDLE_CMD_RESET_DEV:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_reset_dev");
        break;
    case XHCI_HANDLE_CMD_RESET_EP:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_reset_ep");
        break;
    case XHCI_HANDLE_CMD_SET_DEQ:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_set_deq");
        break;
    case XHCI_HANDLE_CMD_SET_DEQ_EP:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_set_deq_ep");
        break;
    case XHCI_HANDLE_CMD_STOP_EP:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_handle_cmd_stop_ep");
        break;
    case XHCI_CONFIGURE_ENDPOINT_CTRL_CTX:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_configure_endpoint_ctrl_ctx");
        break;
    case XHCI_ADDRESS_CTRL_CTX:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_address_ctrl_ctx");
        break;
    case XHCI_DISCOVER_OR_RESET_DEVICE:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_discover_or_reset_device");
        break;
    case XHCI_STOP_DEVICE:
        print_event_slot_ctx((struct xhci_slot_ctx_event_t *)data, "xhci_stop_device");
        break;
