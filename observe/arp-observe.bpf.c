...
int capture_arp(struct xdp_md *ctx) 
{
    ...
    if (data + sizeof(struct ethhdr) + sizeof(struct arphdr) + 2*ETH_ALEN + 2*sizeof(__be32) > data_end)
        return XDP_PASS;
    ...
    if ((unsigned char *)arp_data + 2*ETH_ALEN + 2*sizeof(__be32) > (unsigned char *)data_end) {
        return XDP_PASS;
    }
    ...
    __builtin_memcpy(event.dst_mac, arp_data + ETH_ALEN + sizeof(__be32), ETH_ALEN);
    event.dst_ip = bpf_ntohl(*(__be32 *)(arp_data + ETH_ALEN + sizeof(__be32) + ETH_ALEN));
    
    event.opcode = bpf_ntohs(arp->ar_op);
    event.timestamp = now_ns();
    
    // Reserve space in ring buffer for event
    struct arp_event *ringbuf_space = bpf_ringbuf_reserve(&rb, sizeof(struct arp_event), 0);
    if (!ringbuf_space) {
