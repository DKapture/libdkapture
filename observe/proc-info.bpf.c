...
#define ASSIGN_IO_FIELD(field) io->field = acct->field
    ...
    if (task->pid == 1)
    {
        DEBUG(0, "PID: %d Comm: %s Rchar: %lu Wchar: %lu",
              hdr->pid, hdr->comm, io->rchar, io->wchar);
        DEBUG(0, "Wchar: %llu", task->signal->ioac.wchar);
    }
    if ((task->flags & PF_KTHREAD) && (io->rchar + io->wchar))
    {
        DEBUG(0, "kthread rchar: %lu wchar: %lu",
              io->rchar, io->wchar);
    }
    bpf_ringbuf_submit(hdr, 0);
...
static int dump_proc_statm(struct task_struct *task)
{
    ...
    if (task->mm)
    {
        ...
        statm->resident = statm->shared + task->mm->rss_stat[MM_ANONPAGES].count;
        statm->text = task->mm->end_data - task->mm->start_data;
        statm->text = (statm->text >> PAGE_SHIFT) + !!(statm->text & ((1 << PAGE_SHIFT) - 1));
...
int dump_task_file(struct bpf_iter__task_file *ctx)
{
    ...
    if (0 && task->pid != task->tgid)
    {
         ...
         */
        bpf_info("sub thread passed: tid(%d) tgid(%d)",
                 task->pid, task->tgid);
        return 0;
    }
    ...
    // 打印 fd 的所有字段
    DEBUG(0, "task: %s pid: %d\n"
             "\ti_mode: %d\n"
             "\tfd: %d\n"
             "\tinode: %lu\n"
             "\tdev: %u:%u",
          hdr->comm, hdr->pid,
          fd->i_mode, fd->fd, fd->inode,
...
int BPF_PROG(fr_sock_write_iter,
             struct kiocb *iocb,
...
int BPF_PROG(fr___sys_sendto_exit,
             int fd, void __user *buff,
...
int BPF_PROG(fr_____sys_sendmsg,
             struct socket *sock,
