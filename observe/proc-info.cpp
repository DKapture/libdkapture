...
{
    const struct DKapture::DataHdr *hdr = static_cast<const struct DKapture::DataHdr *>(data);
    ...
    if (env.verbose)
    {
        std::cerr << "Received event #" << ++event_count << ", type: " << hdr->type 
                  << ", size: " << data_sz << " bytes, pid: " << hdr->pid 
    ...
    case DKapture::PROC_PID_STAT:
    {
        ...
        std::cout << "PPID:" << stat->ppid 
                  << " PGID:" << stat->pgid 
                  << " SID:" << stat->sid 
                  << " NICE:" << stat->nice << std::endl;
        break;
    }
    
    case DKapture::PROC_PID_IO:
    {
        if (!env.show_io) break;
        const struct ProcPidIo *io = reinterpret_cast<const struct ProcPidIo *>(hdr->data);
    ...
    {
        if (!env.show_traffic) break;
        const struct ProcPidTraffic *traffic = reinterpret_cast<const struct ProcPidTraffic *>(hdr->data);
    ...
    {
        if (!env.show_statm) break;
        const struct ProcPidStatm *statm = reinterpret_cast<const struct ProcPidStatm *>(hdr->data);
    ...
    {
        if (!env.show_status) break;
        const struct ProcPidStatus *status = reinterpret_cast<const struct ProcPidStatus *>(hdr->data);
    ...
    {
        if (!env.show_schedstat) break;
        const struct ProcPidSchedstat *schedstat = reinterpret_cast<const struct ProcPidSchedstat *>(hdr->data);
    ...
    {
        const struct ProcPidFd *fd = reinterpret_cast<const struct ProcPidFd *>(hdr->data);
...
int main(int argc, char **argv)
{
    ...
    rb = ring_buffer__new(bpf_map__fd(obj->maps.dk_shared_mem), handle_event, NULL, NULL);
    if (!rb)
    {
