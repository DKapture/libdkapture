...
int dump_task(struct bpf_iter__task *ctx)
{
    ...
    dtask->rss = 0;
    if (mm) {
        dtask->rss += BPF_CORE_READ(task, mm, rss_stat[MM_FILEPAGES].count) * (PAGE_SIZE >> 10);
        dtask->rss += BPF_CORE_READ(task, mm, rss_stat[MM_ANONPAGES].count) * (PAGE_SIZE >> 10);
        dtask->rss += BPF_CORE_READ(task, mm, rss_stat[MM_SHMEMPAGES].count) * (PAGE_SIZE >> 10);
    }

    dtask->rsslim = BPF_CORE_READ(task, signal, rlim[5/*RLIMIT_RSS*/].rlim_cur);
    ...
    dtask->resident = dtask->shared + BPF_CORE_READ(task, mm, rss_stat)[MM_ANONPAGES].count;
