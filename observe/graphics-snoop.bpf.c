...
static __always_inline void fill_common_header(
    struct graphics_event_header *header, __u32 event_type)
{
...
int handle_dma_fence_init(struct dma_fence_init_ctx *ctx)
{
    ...
    char *driver_str = (char *)((void *)ctx + (ctx->__data_loc_driver & 0xffff));
    char *timeline_str = (char *)((void *)ctx + (ctx->__data_loc_timeline & 0xffff));
    bpf_probe_read_kernel_str(event.data.fence.driver_name, 
                             sizeof(event.data.fence.driver_name), driver_str);
    bpf_probe_read_kernel_str(event.data.fence.timeline_name,
                             sizeof(event.data.fence.timeline_name), timeline_str);

    /* 记录围栏创建 */
    bpf_map_update_elem(&fence_tracker, &event.data.fence.fence_ptr, &now, BPF_ANY);
