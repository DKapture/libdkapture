...
int tp_xhci_alloc_dev(struct tp_xhci_alloc_dev_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_alloc_dev: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, tt_info: %u, state: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.tt_info, event.state);
...
int tp_xhci_free_dev(struct tp_xhci_free_dev_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_free_dev: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, tt_info: %u, state: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.tt_info, event.state);
...
int tp_xhci_urb_enqueue(struct tp_xhci_urb_enqueue_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_urb_enqueue: pid: %d, tid: %d, comm: %s, urb: %llx, ep%d%s, slot: %d, len: %d/%d\n",
        event.pid, event.tid, event.comm, event.urb, event.epnum, 
        event.dir_in ? "in" : "out", event.slot_id, event.actual, event.length);
...
int tp_xhci_urb_giveback(struct tp_xhci_urb_giveback_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_urb_giveback: pid: %d, tid: %d, comm: %s, urb: %llx, ep%d%s, slot: %d, len: %d/%d, status: %d\n",
        event.pid, event.tid, event.comm, event.urb, event.epnum, 
        event.dir_in ? "in" : "out", event.slot_id, event.actual, event.length, event.status);
...
int tp_xhci_handle_event(struct tp_xhci_handle_event_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_handle_event: pid: %d, tid: %d, comm: %s, type: %u, fields: %u %u %u %u\n",
        event.pid, event.tid, event.comm, event.type, event.field0, event.field1, event.field2, event.field3);
...
int tp_xhci_handle_transfer(struct tp_xhci_handle_transfer_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_handle_transfer: pid: %d, tid: %d, comm: %s, type: %u, fields: %u %u %u %u\n",
        event.pid, event.tid, event.comm, event.type, event.field0, event.field1, event.field2, event.field3);
...
int tp_xhci_queue_trb(struct tp_xhci_queue_trb_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_queue_trb: pid: %d, tid: %d, comm: %s, type: %u, fields: %u %u %u %u\n",
        event.pid, event.tid, event.comm, event.type, event.field0, event.field1, event.field2, event.field3);
...
int tp_xhci_setup_device(struct tp_xhci_setup_device_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_setup_device: pid: %d, tid: %d, comm: %s, vdev: %llx, devnum: %d, state: %d, speed: %d, port: %d, slot: %d\n",
        event.pid, event.tid, event.comm, event.vdev, event.devnum, event.state, event.speed, event.portnum, event.slot_id);
...
int tp_xhci_ring_alloc(struct tp_xhci_ring_alloc_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_ring_alloc: pid: %d, tid: %d, comm: %s, type: %u, ring: %llx, segs: %u, stream: %u\n",
        event.pid, event.tid, event.comm, event.type, event.ring, event.num_segs, event.stream_id);
...
int tp_xhci_add_endpoint(struct tp_xhci_add_endpoint_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_add_endpoint: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, deq: %llx, tx_info: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.deq, event.tx_info);
...
int tp_xhci_address_ctx(struct tp_xhci_address_ctx_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_address_ctx: pid: %d, tid: %d, comm: %s, ctx_64: %d, type: %u, dma: %llx, va: %llx\n",
        event.pid, event.tid, event.comm, event.ctx_64, event.ctx_type, event.ctx_dma, event.ctx_va);
...
int tp_xhci_alloc_virt_device(struct tp_xhci_alloc_virt_device_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_alloc_virt_device: pid: %d, tid: %d, comm: %s, vdev: %llx, devnum: %d, state: %d, speed: %d, port: %d, slot: %d\n",
        event.pid, event.tid, event.comm, event.vdev, event.devnum, event.state, event.speed, event.portnum, event.slot_id);
...
int tp_xhci_configure_endpoint(struct tp_xhci_configure_endpoint_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_configure_endpoint: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, tt_info: %u, state: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.tt_info, event.state);
...
int tp_xhci_ring_free(struct tp_xhci_ring_free_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_ring_free: pid: %d, tid: %d, comm: %s, type: %u, ring: %llx, segs: %u, stream: %u\n",
        event.pid, event.tid, event.comm, event.type, event.ring, event.num_segs, event.stream_id);
...
int tp_xhci_inc_deq(struct tp_xhci_inc_deq_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_inc_deq: pid: %d, tid: %d, comm: %s, type: %u, ring: %llx, segs: %u, stream: %u\n",
        event.pid, event.tid, event.comm, event.type, event.ring, event.num_segs, event.stream_id);
...
int tp_xhci_inc_enq(struct tp_xhci_inc_enq_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_inc_enq: pid: %d, tid: %d, comm: %s, type: %u, ring: %llx, segs: %u, stream: %u\n",
        event.pid, event.tid, event.comm, event.type, event.ring, event.num_segs, event.stream_id);
...
int tp_xhci_ring_ep_doorbell(struct tp_xhci_ring_ep_doorbell_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_ring_ep_doorbell: pid: %d, tid: %d, comm: %s, slot: %u, doorbell: %u\n",
...
int tp_xhci_urb_dequeue(struct tp_xhci_urb_dequeue_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_urb_dequeue: pid: %d, tid: %d, comm: %s, urb: %llx, ep%d%s, slot: %d, len: %d/%d, status: %d\n",
        event.pid, event.tid, event.comm, event.urb, event.epnum, 
        event.dir_in ? "in" : "out", event.slot_id, event.actual, event.length, event.status);
...
int tp_xhci_setup_addressable_virt_device(struct tp_xhci_alloc_virt_device_ctx_t *ctx)
{
    long ret = 0;
    struct xhci_alloc_virt_device_event_t event = {0};
    ...
    DEBUG(0, "xhci_setup_addressable_virt_device: pid: %d, tid: %d, comm: %s, vdev: %llx, devnum: %d, state: %d, speed: %d, port: %d, slot: %d\n",
        event.pid, event.tid, event.comm, event.vdev, event.devnum, event.state, event.speed, event.portnum, event.slot_id);
...
int tp_xhci_free_virt_device(struct tp_xhci_alloc_virt_device_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_free_virt_device: pid: %d, tid: %d, comm: %s, vdev: %llx, devnum: %d, state: %d, speed: %d, port: %d, slot: %d\n",
        event.pid, event.tid, event.comm, event.vdev, event.devnum, event.state, event.speed, event.portnum, event.slot_id);
...
int tp_xhci_setup_device_slot(struct tp_xhci_slot_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_setup_device_slot: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, tt_info: %u, state: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.tt_info, event.state);
...
int tp_xhci_handle_cmd_addr_dev(struct tp_xhci_slot_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_handle_cmd_addr_dev: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, tt_info: %u, state: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.tt_info, event.state);
...
int tp_xhci_handle_cmd_config_ep(struct tp_xhci_slot_ctx_t *ctx)
{
    ...
    DEBUG(0, "xhci_handle_cmd_config_ep: pid: %d, tid: %d, comm: %s, info: %u, info2: %u, tt_info: %u, state: %u\n",
        event.pid, event.tid, event.comm, event.info, event.info2, event.tt_info, event.state);
