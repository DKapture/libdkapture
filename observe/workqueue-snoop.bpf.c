...
static __always_inline bool should_filter_event(struct filter_config *cfg, 
    ...
    if (cfg->filter_workqueue && workqueue_name) {
        ...
        for (int i = 0; i < WORKQUEUE_NAME_LEN-1; i++) {
            if (cfg->target_workqueue[i] == 0) break;
...
static __always_inline void send_event(u8 event_type, u64 work_ptr, 
    ...
    struct workqueue_event *event = bpf_ringbuf_reserve(&events, sizeof(*event), 0);
    if (!event) return;
    ...
    if (workqueue_name) {
        bpf_probe_read_str(event->workqueue_name, sizeof(event->workqueue_name), workqueue_name);
    } else {
...
int tp_workqueue_queue_work(struct trace_event_raw_workqueue_queue_work *ctx) {
    ...
    if (workqueue_name) {
        bpf_probe_read_str(timing.workqueue_name, sizeof(timing.workqueue_name), workqueue_name);
    }
    
    bpf_map_update_elem(&work_timings, &work_ptr, &timing, BPF_ANY);
    
    send_event(WQ_EVENT_QUEUE, work_ptr, function_ptr, workqueue_name, req_cpu, 0);
    return 0;
}

SEC("tp/workqueue/workqueue_activate_work")  
int tp_workqueue_activate_work(struct trace_event_raw_workqueue_activate_work *ctx) {
    u64 work_ptr = (u64)ctx->work;
...
int tp_workqueue_execute_start(struct trace_event_raw_workqueue_execute_start *ctx) {
    u64 work_ptr = (u64)ctx->work;
    u64 function_ptr = (u64)ctx->function;
