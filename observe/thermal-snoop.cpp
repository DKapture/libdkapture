...
static const struct argp_option opts[] = {
    ...
    { "min-temp", ARG_MIN_TEMP, "TEMP", 0, "Minimum temperature to trace (millicelsius)", 0 },
    { "max-temp", ARG_MAX_TEMP, "TEMP", 0, "Maximum temperature to trace (millicelsius)", 0 },
    { "verbose", 'v', NULL, 0, "Verbose output", 0 },
    { "timestamp", 't', NULL, 0, "Show timestamps", 0 },
    { "stats", 's', NULL, 0, "Show statistics", 0 },
    { "celsius", ARG_CELSIUS, NULL, 0, "Display temperatures in Celsius (default)", 0 },
...
{
    switch (event_type) {
    case THERMAL_TEMP_UPDATE:    return "TEMP_UPDATE";
    case THERMAL_TRIP_TRIGGERED: return "TRIP_TRIGGER";
    case THERMAL_CDEV_UPDATE:    return "CDEV_UPDATE";
    case THERMAL_POWER_ALLOC:    return "POWER_ALLOC";
    case THERMAL_POWER_PID:      return "POWER_PID";
...
static const char *format_temperature(__s32 milli_temp)
{
    ...
    if (env.celsius) {
        snprintf(temp_str, sizeof(temp_str), "%.1f°C", (double)milli_temp / 1000.0);
    } else {
        snprintf(temp_str, sizeof(temp_str), "%.1f°F", ((double)milli_temp / 1000.0) * 9.0 / 5.0 + 32.0);
    }
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    case THERMAL_TEMP_UPDATE:
        ...
        if (global_stats.min_temp_seen == 0 || e->data.temp_update.temperature < global_stats.min_temp_seen)
        ...
        print_timestamp();
        if (env.verbose) {
            printf("CPU[%u] PID:%u %s\n", e->header.cpu, e->header.pid, e->header.comm);
            printf("  Event: TEMP_UPDATE\n");
            printf("  Zone: zone%u (%s)\n", e->data.temp_update.thermal_zone_id, e->data.temp_update.zone_type);
            printf("  Temperature: %s -> %s\n", 
                   format_temperature(e->data.temp_update.prev_temp),
                   format_temperature(e->data.temp_update.temperature));
            printf("  Change: %+.1f°C\n", 
                   (double)(e->data.temp_update.temperature - e->data.temp_update.prev_temp) / 1000.0);
            printf("\n");
        } else {
            printf("[%u] %-12s zone%u (%s): %s -> %s\n",
                   e->header.cpu, "TEMP_UPDATE", e->data.temp_update.thermal_zone_id,
                   e->data.temp_update.zone_type, format_temperature(e->data.temp_update.prev_temp),
                   format_temperature(e->data.temp_update.temperature));
        }
        break;
    ...
    case THERMAL_TRIP_TRIGGERED:
        ...
        if (env.verbose) {
            ...
            printf("  Zone: zone%u (ID: %u)\n", e->data.trip_event.thermal_zone_id, e->data.trip_event.trip_id);
            ...
            printf("[%u] %-12s zone%u: %s trip at %s (current: %s)\n",
                   e->header.cpu, "TRIP_TRIGGER", e->data.trip_event.thermal_zone_id,
                   trip_type_description(e->data.trip_event.trip_type),
                   format_temperature(e->data.trip_event.trip_temp),
                   format_temperature(e->data.trip_event.current_temp));
        }
    ...
    case THERMAL_CDEV_UPDATE:
        ...
        if (env.verbose) {
            ...
            printf("  Device: %s (ID: %u)\n", e->data.cdev_update.cdev_type, e->data.cdev_update.cdev_id);
            printf("  State: %u -> %u (max: %u)\n", 
                   e->data.cdev_update.old_state, e->data.cdev_update.new_state, e->data.cdev_update.max_state);
            printf("  Power: %llu mW\n", e->data.cdev_update.power);
            printf("\n");
        } else {
            printf("[%u] %-12s %s: state %u -> %u (max: %u)\n",
                   e->header.cpu, "CDEV_UPDATE", cdev_type_description(e->data.cdev_update.cdev_type),
                   e->data.cdev_update.old_state, e->data.cdev_update.new_state, e->data.cdev_update.max_state);
        }
        break;
    ...
    case THERMAL_POWER_ALLOC:
        ...
        if (env.verbose) {
            ...
            printf("[%u] %-12s zone%u: req=%umW granted=%umW delta=%s\n",
                   e->header.cpu, "POWER_ALLOC", e->data.power_alloc.thermal_zone_id,
                   e->data.power_alloc.total_req_power, e->data.power_alloc.granted_power,
                   format_temperature(e->data.power_alloc.delta_temp));
        }
        break;
    ...
    case THERMAL_POWER_PID:
        ...
        if (env.verbose) {
            ...
            printf("[%u] %-12s zone%u: err=%s P=%d I=%d D=%d out=%d\n",
                   e->header.cpu, "POWER_PID", e->data.power_pid.thermal_zone_id,
                   format_temperature(e->data.power_pid.err),
                   e->data.power_pid.p_term, e->data.power_pid.i_term,
                   e->data.power_pid.d_term, e->data.power_pid.output);
        }
        break;
...
static void print_stats()
{
    ...
    printf("├─ Temperature Updates: %llu (%.1f%%)\n", 
           global_stats.temp_update_events,
           global_stats.total_events ? (double)global_stats.temp_update_events * 100.0 / global_stats.total_events : 0.0);
    printf("├─ Trip Triggers:       %llu (%.1f%%)\n", 
           global_stats.trip_events,
           global_stats.total_events ? (double)global_stats.trip_events * 100.0 / global_stats.total_events : 0.0);
    printf("├─ Cooling Dev Updates: %llu (%.1f%%)\n", 
           global_stats.cdev_update_events,
           global_stats.total_events ? (double)global_stats.cdev_update_events * 100.0 / global_stats.total_events : 0.0);
    printf("├─ Power Allocator:     %llu (%.1f%%)\n", 
           global_stats.power_alloc_events,
           global_stats.total_events ? (double)global_stats.power_alloc_events * 100.0 / global_stats.total_events : 0.0);
    printf("└─ Power PID Control:   %llu (%.1f%%)\n", 
           global_stats.power_pid_events,
           global_stats.total_events ? (double)global_stats.power_pid_events * 100.0 / global_stats.total_events : 0.0);
    printf("\n");
...
#endif
{
    ...
    /* Setup filter if needed */
    {
        ...
        bpf_map__update_elem(obj->maps.filter_map, &key, sizeof(key), &filter, sizeof(filter), BPF_ANY);
    }
    
    /* Setup ring buffer polling */
#ifdef BUILTIN
    rb = ring_buffer__new(bpf_map__fd(obj->maps.thermal_events), callback, NULL, NULL);
#else
    rb = ring_buffer__new(bpf_map__fd(obj->maps.thermal_events), handle_event, NULL, NULL);
#endif
    if (!rb) {
        err = -1;
