...
static __always_inline void fill_common_header(struct thermal_event_header *header, __u32 event_type)
{
...
static __always_inline bool is_significant_temp_change(__u32 zone_id, __s32 new_temp)
{
...
int handle_thermal_zone_trip(void *ctx)
{
    ...
    __builtin_memcpy(event.data.trip_event.trip_type, "unknown", 8);  /* Extract from ctx */
...
int handle_cdev_update(void *ctx)
{
    ...
    __builtin_memcpy(event.data.cdev_update.cdev_type, "unknown", 8);  /* Extract from ctx */
    ...
    bpf_map_update_elem(&cdev_state_history, &cdev_id, &event.data.cdev_update.new_state, BPF_ANY);
