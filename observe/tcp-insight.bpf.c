...
/* Helper functions */
static __always_inline __u64 generate_conn_id(__u32 saddr, __u16 sport, __u32 daddr, __u16 dport) {
    return ((__u64)saddr << 32) | ((__u64)sport << 16) | ((__u64)daddr >> 16) | dport;
}
...
static __always_inline bool should_trace_addr(__u32 saddr, __u32 daddr, __u16 sport, __u16 dport) {
    struct tcp_filter_rule *rule;
    __u32 key = 0;
...
int trace_inet_sock_set_state(struct tp_inet_sock_set_state *ctx)
{
    ...
    if (!conn) {
        ...
        if (ctx->newstate == TCP_ESTABLISHED) {
            __u32 key = 0;
            struct tcp_global_stats *stats = bpf_map_lookup_elem(&global_stats, &key);
            if (stats)
