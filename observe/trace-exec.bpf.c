...
int BPF_KPROBE(bprm_execve, struct linux_binprm *bprm, int fd, struct filename *filename, int flags)
{
	...
	ret = bpf_map_update_elem(&args_map, &pid, &args, BPF_ANY); // Update args map
	if (ret != 0)
	{
		bpf_printk("error: bpf_map_update_elem(%ld): %d", pid, ret); // Log error
	}
...
int BPF_KRETPROBE(bprm_execve_ret, int _ret)
{
	...
	ret = bpf_map_update_elem(&mem_pool, &key, &buf_mirror, BPF_ANY); // Update memory pool
	if (ret != 0)
	{
		bpf_printk("error: fail to call bpf_map_update_elem: %d", ret); // Log error
		goto exit;
	}
	struct Buf *buf = bpf_map_lookup_elem(&mem_pool, &key); // Lookup buffer
	if (buf == NULL)
	{
		bpf_printk("fail to call bpf_map_lookup_elem: %d", ret); // Log error
		goto exit;
	}
	ret = bpf_probe_read_kernel(buf->path, PATH_MAX, args->filename->iname); // Read filename
	if (ret)
	{
	...
	ret = bpf_probe_read_kernel(filepath, 16, &current->comm); // Read command name
	if (ret)
	{
	...
	do
	{
		ret = bpf_probe_read_kernel(&current, sizeof(current), &current->real_parent); // Read parent task
		if (ret)
		{
			bpf_printk("fail to read parent: %d", ret); // Log error
			break;
		}
		ret = bpf_probe_read_kernel(&pid, sizeof(pid), &current->pid); // Read parent PID
		if (ret)
		{
		...
		ret = bpf_probe_read_kernel(comm, sizeof(comm), &current->comm); // Read command name
		if (ret)
		{
			bpf_printk("fail to read comm: %d", ret); // Log error
			break;
