...
static void print_title(int max_name_len)
{
    ...
    printf("%-*s %-10s %-10s %-10s\n",
           max_name_len, "SYSCALL", "CALL-CNT", "ERROR", "SECONDS");
...
void* timer_task(void*)
{
    ...
    while (!exit_flag)
    {
        u32 key = 0, nxt_key;
        u32 total = 0;
        std::vector<std::pair<u32, info>> stats; // Vector to store syscall stats
        ...
        std::sort(stats.begin(), stats.end(), 
            [](const std::pair<u32, info> &a, const std::pair<u32, info> &b)
        ...
        for (const auto &stat : stats)
        {
            ...
            printf("%-*s %-10llu %-10ld %-f\n",
                   max_name_len, sys_tbl[stat.first],
                   info.cnt, info.ret, info.time / info.cnt / 1000000000.0f);
        }
...
int main(int argc, char *args[])
{
    ...
    printf("\tpid: %d, pathhash: %d, comm: %s\n\n",
           rule.pid, rule.pathhash, rule.comm);
