...
static int send_event(u32 event_type, struct task_struct *task, void *extra_data)
{
    u32 key = 0;
    ...
    if (extra_data) {
        switch (event_type) {
            case SCHED_WAKEUP:
                __builtin_memcpy(&log->wakeup_data, extra_data, sizeof(log->wakeup_data));
                break;
            case SCHED_MIGRATE:
                __builtin_memcpy(&log->migrate_data, extra_data, sizeof(log->migrate_data));
                break;
            case SCHED_FORK:
                __builtin_memcpy(&log->fork_data, extra_data, sizeof(log->fork_data));
                break;
            ...
            case SCHED_STAT_IOWAIT:
                __builtin_memcpy(&log->stat_data, extra_data, sizeof(log->stat_data));
                break;
        }
    }
...
int handle_sched_switch(struct trace_event_raw_sched_switch *ctx)
{
    ...
    bpf_probe_read_kernel_str(log->switch_data.prev_comm, TASK_COMM_LEN, ctx->prev_comm);
    log->switch_data.prev_pid = prev_pid;
    log->switch_data.prev_prio = ctx->prev_prio;
    log->switch_data.prev_state = ctx->prev_state;
    
    bpf_probe_read_kernel_str(log->switch_data.next_comm, TASK_COMM_LEN, ctx->next_comm);
...
int handle_sched_fork(struct trace_event_raw_sched_process_fork *ctx)
{
    ...
    bpf_probe_read_kernel_str(log->fork_data.parent_comm, TASK_COMM_LEN, ctx->parent_comm);
    log->fork_data.parent_pid = parent_pid;
    bpf_probe_read_kernel_str(log->fork_data.child_comm, TASK_COMM_LEN, ctx->child_comm);
