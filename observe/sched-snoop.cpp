...
{
    switch (event_type) {
        case SCHED_SWITCH: return "SWITCH";
        case SCHED_WAKEUP:  return "WAKEUP";
        case SCHED_WAKEUP_NEW: return "WAKEUP_NEW";
        case SCHED_MIGRATE: return "MIGRATE";
        case SCHED_FORK:    return "FORK";
        case SCHED_EXIT:    return "EXIT";
        case SCHED_EXEC:    return "EXEC";
        case SCHED_STAT_RUNTIME: return "STAT_RT";
        case SCHED_STAT_WAIT: return "STAT_WAIT";
        case SCHED_STAT_SLEEP: return "STAT_SLEEP";
        case SCHED_STAT_BLOCKED: return "STAT_BLOCK";
        case SCHED_STAT_IOWAIT: return "STAT_IOWAIT";
...
static const char* state_to_str(uint64_t state)
{
    switch (state) {
        ...
        case 0x0001: return "S";  // Interruptible sleep
        case 0x0002: return "D";  // Uninterruptible sleep
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    if (rule.target_comm[0] != '\0') {
        ...
        if (!match_found && log->event_type == SCHED_SWITCH) {
            if (strstr(log->switch_data.prev_comm, rule.target_comm) != nullptr ||
        ...
        if (!match_found && log->event_type == SCHED_FORK) {
            if (strstr(log->fork_data.parent_comm, rule.target_comm) != nullptr ||
...
int main(int argc, char **argv)
{
    ...
    // Set filter rules
    uint32_t key = 0;
    err = bpf_map_update_elem(bpf_map__fd(skel->maps.filter), &key, &rule, BPF_ANY);
    if (err) {
    ...
    rb = ring_buffer__new(bpf_map__fd(skel->maps.logs), handle_event, nullptr, nullptr);
    if (!rb) {
