...
static int print_stack(struct pagefault_bpf *skel, int stack_id) {
    ...
    for (int i = 0; i < 128 && ips[i]; i++) {
        snprintf(sym, sizeof(sym), "        0x%llx", (unsigned long long)ips[i]);
        printf("%s\n", sym);
    }
    return 0;
}
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    ...
    if (env.pid && e->pid != env.pid)
        return 0;
    printf("PID:%d TID:%d COMM:%s ADDR:0x%lx IP:0x%lx ERR:0x%lx",
           e->pid, e->tid, e->comm, e->address, e->ip, e->error_code);
    if (env.show_ts)
...
int main(int argc, char **argv) {
    ...
    // 过滤pid
    if (env.pid)
        bpf_map_update_elem(bpf_map__fd(skel->maps.filter), &key, &env.pid, BPF_ANY);
