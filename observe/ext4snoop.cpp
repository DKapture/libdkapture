...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	case EXT4_ALLOC_DA_BLOCKS: {
		...
		printf("%-14s dev=%u:%u ino=%lu data_blocks=%u",
		       "-alloc-da-blocks", major(event->dev), minor(event->dev),
	...
	case EXT4_ALLOCATE_BLOCKS: {
		...
		printf("%-14s dev=%u:%u ino=%lu block=%llu len=%u logical=%u lleft=%u lright=%u goal=%llu pleft=%llu pright=%llu flags=%u",
		       "-allocate-blocks", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino,
		       (unsigned long long)event->block, event->len,
		       event->logical, event->lleft, event->lright,
		       (unsigned long long)event->goal,
		       (unsigned long long)event->pleft,
	...
	case EXT4_ALLOCATE_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu dir=%lu mode=%o",
		       "-allocate-inode", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (unsigned long)event->dir,
		       event->mode);
		break;
	}
	case EXT4_BEGIN_ORDERED_TRUNCATE: {
		...
		printf("%-14s dev=%u:%u ino=%lu new_size=%lld",
		       "-begin-ordered-truncate", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino,
		       (long long)event->new_size);
		break;
	}
	case EXT4_COLLAPSE_RANGE: {
		...
		printf("%-14s dev=%u:%u ino=%lu offset=%lld len=%lld",
		       "-collapse-range", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (long long)event->offset,
		       (long long)event->len);
		break;
	}
	case EXT4_DA_RELEASE_SPACE: {
		...
		printf("%-14s dev=%u:%u ino=%lu i_blocks=%llu freed_blocks=%d reserved_data_blocks=%d mode=%o",
		       "-da-release-space", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino,
		       (unsigned long long)event->i_blocks, event->freed_blocks,
	...
	case EXT4_DA_RESERVE_SPACE: {
		...
		printf("%-14s dev=%u:%u ino=%lu i_blocks=%llu reserved_data_blocks=%d mode=%o",
		       "-da-reserve-space", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino,
		       (unsigned long long)event->i_blocks,
		       event->reserved_data_blocks, event->mode);
		break;
	}
	case EXT4_DA_UPDATE_RESERVE_SPACE: {
		...
		printf("%-14s dev=%u:%u ino=%lu i_blocks=%llu used_blocks=%d reserved_data_blocks=%d quota_claim=%d mode=%o",
		       "-da-update-reserve-space", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino,
		       (unsigned long long)event->i_blocks, event->used_blocks,
		       event->reserved_data_blocks, event->quota_claim,
		       event->mode);
		break;
	}
	case EXT4_DA_WRITE_BEGIN: {
		...
		printf("%-14s dev=%u:%u ino=%lu pos=%lld len=%u",
		       "-da-write-begin", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (long long)event->pos,
		       event->len);
		break;
	}
	case EXT4_DA_WRITE_END: {
		...
		printf("%-14s dev=%u:%u ino=%lu pos=%lld len=%u copied=%u",
		       "-da-write-end", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (long long)event->pos,
	...
	case EXT4_DA_WRITE_PAGES: {
		...
		printf("%-14s dev=%u:%u ino=%lu first_page=%lu nr_to_write=%ld sync_mode=%d",
		       "-da-write-pages", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, event->first_page,
		       event->nr_to_write, event->sync_mode);
		break;
	}
	case EXT4_DA_WRITE_PAGES_EXTENT: {
		...
		printf("%-14s dev=%u:%u ino=%lu lblk=%llu len=%u flags=%u",
		       "-da-write-pages-extent", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino,
	...
	case EXT4_DISCARD_BLOCKS: {
		...
		printf("%-14s dev=%u:%u blk=%llu count=%llu", "-discard-blocks",
		       major(event->dev), minor(event->dev),
		       (unsigned long long)event->blk,
	...
	case EXT4_DISCARD_PREALLOCATIONS: {
		...
		printf("%-14s dev=%u:%u ino=%lu len=%u needed=%u",
		       "-discard-preallocations", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino, event->len,
		       event->needed);
		break;
	}
	case EXT4_DROP_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu drop=%d", "-drop-inode",
		       major(event->dev), minor(event->dev),
	...
	case EXT4_ERROR: {
		...
		printf("%-14s dev=%u:%u function=%s line=%u", "-error",
		       major(event->dev), minor(event->dev), event->function,
		       event->line);
		break;
	}
	case EXT4_ES_CACHE_EXTENT: {
		...
		printf("%-14s dev=%u:%u ino=%lu lblk=%u len=%u pblk=%llu status=%c",
		       "-es-cache-extent", major(event->dev), minor(event->dev),
	...
	case EXT4_ES_FIND_EXTENT_RANGE_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-find-extent-range-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_FIND_EXTENT_RANGE_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-find-extent-range-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_INSERT_DELAYED_BLOCK: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-insert-delayed-block",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_INSERT_EXTENT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-insert-extent",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_LOOKUP_EXTENT_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-lookup-extent-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_LOOKUP_EXTENT_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-lookup-extent-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_REMOVE_EXTENT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-es-remove-extent",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_ES_SHRINK: {
		...
		printf("%-14s dev=%u:%u nr_shrunk=%d scan_time=%llu",
		       "-es-shrink", major(event->dev), minor(event->dev),
	...
	case EXT4_ES_SHRINK_COUNT: {
		...
		printf("%-14s dev=%u:%u nr_to_scan=%d cache_cnt=%d",
		       "-es-shrink-count", major(event->dev), minor(event->dev),
	...
	case EXT4_ES_SHRINK_SCAN_ENTER: {
		...
		printf("%-14s dev=%u:%u nr_to_scan=%d cache_cnt=%d",
		       "-es-shrink-scan-enter", major(event->dev),
		       minor(event->dev), event->nr_to_scan, event->cache_cnt);
		break;
	}
	case EXT4_ES_SHRINK_SCAN_EXIT: {
		...
		printf("%-14s dev=%u:%u nr_shrunk=%d cache_cnt=%d",
		       "-es-shrink-scan-exit", major(event->dev),
		       minor(event->dev), event->nr_shrunk, event->cache_cnt);
		break;
	}
	case EXT4_EVICT_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu nlink=%d", "-evict-inode",
		       major(event->dev), minor(event->dev),
	...
	case EXT4_EXT_CONVERT_TO_INITIALIZED_ENTER: {
		if (data_sz < sizeof(ext4_ext_convert_to_initialized_enter_t))
			return 0;
		ext4_ext_convert_to_initialized_enter_t *event =
			(typeof(event))data;
		printf("%-14s dev=%u:%u ino=%lu",
		       "-ext-convert-to-initialized-enter", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_CONVERT_TO_INITIALIZED_FASTPATH: {
		if (data_sz <
		    sizeof(ext4_ext_convert_to_initialized_fastpath_t))
			return 0;
		ext4_ext_convert_to_initialized_fastpath_t *event =
			(typeof(event))data;
		printf("%-14s dev=%u:%u ino=%lu",
	...
	case EXT4_EXT_HANDLE_UNWRITTEN_EXTENTS: {
		if (data_sz < sizeof(ext4_ext_handle_unwritten_extents_t))
			return 0;
		ext4_ext_handle_unwritten_extents_t *event =
			(typeof(event))data;
		printf("%-14s dev=%u:%u ino=%lu",
		       "-ext-handle-unwritten-extents", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_LOAD_EXTENT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-load-extent",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_MAP_BLOCKS_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-map-blocks-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_MAP_BLOCKS_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-map-blocks-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_REMOVE_SPACE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-remove-space",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_REMOVE_SPACE_DONE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-remove-space-done",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_RM_IDX: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-rm-idx",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_RM_LEAF: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-rm-leaf",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_EXT_SHOW_EXTENT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ext-show-extent",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_FALLOCATE_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-fallocate-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_FALLOCATE_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-fallocate-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_FC_CLEANUP: {
		...
		printf("%-14s dev=%u:%u j_fc_off=%d full=%d tid=%u",
		       "-fc-cleanup", major(event->dev), minor(event->dev),
	...
	case EXT4_FC_COMMIT_START: {
		...
		printf("%-14s dev=%u:%u tid=%u", "-fc-commit-start",
		       major(event->dev), minor(event->dev), event->tid);
		break;
	}
	case EXT4_FC_COMMIT_STOP: {
		...
		printf("%-14s dev=%u:%u nblks=%d reason=%d tid=%u",
		       "-fc-commit-stop", major(event->dev), minor(event->dev),
	...
	case EXT4_FC_REPLAY: {
		...
		printf("%-14s dev=%u:%u tag=%d ino=%d", "-fc-replay",
		       major(event->dev), minor(event->dev), event->tag,
		       event->ino);
		break;
	}
	case EXT4_FC_REPLAY_SCAN: {
		...
		printf("%-14s dev=%u:%u error=%d off=%d", "-fc-replay-scan",
		       major(event->dev), minor(event->dev), event->error,
		       event->off);
		break;
	}
	case EXT4_FC_STATS: {
		...
		printf("%-14s dev=%u:%u fc_commits=%lu fc_ineligible_commits=%lu",
		       "-fc-stats", major(event->dev), minor(event->dev),
	...
	case EXT4_FC_TRACK_CREATE: {
		...
		printf("%-14s dev=%u:%u ino=%lu tid=%u", "-fc-track-create",
		       major(event->dev), minor(event->dev),
	...
	case EXT4_FC_TRACK_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu tid=%u", "-fc-track-inode",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->i_ino, event->t_tid);
		break;
	}
	case EXT4_FC_TRACK_LINK: {
		...
		printf("%-14s dev=%u:%u ino=%lu tid=%u", "-fc-track-link",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->i_ino, event->t_tid);
		break;
	}
	case EXT4_FC_TRACK_RANGE: {
		...
		printf("%-14s dev=%u:%u ino=%lu tid=%u start=%ld end=%ld",
		       "-fc-track-range", major(event->dev), minor(event->dev),
		       (unsigned long)event->i_ino, event->t_tid, event->start,
		       event->end);
		break;
	}
	case EXT4_FC_TRACK_UNLINK: {
		...
		printf("%-14s dev=%u:%u ino=%lu tid=%u", "-fc-track-unlink",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->i_ino, event->t_tid);
		break;
	}
	case EXT4_FORGET: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-forget", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino);
		break;
	}
	case EXT4_FREE_BLOCKS: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-free-blocks",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_FREE_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-free-inode",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_FSMAP_HIGH_KEY: {
		...
		printf("%-14s dev=%u:%u keydev=%u:%u agno=%u bno=%llu",
		       "-fsmap-high-key", major(event->dev), minor(event->dev),
		       major(event->keydev), minor(event->keydev), event->agno,
		       event->bno);
		break;
	}
	case EXT4_FSMAP_LOW_KEY: {
		...
		printf("%-14s dev=%u:%u keydev=%u:%u agno=%u bno=%llu",
		       "-fsmap-low-key", major(event->dev), minor(event->dev),
		       major(event->keydev), minor(event->keydev), event->agno,
		       event->bno);
		break;
	}
	case EXT4_FSMAP_MAPPING: {
		...
		printf("%-14s dev=%u:%u keydev=%u:%u agno=%u bno=%llu",
		       "-fsmap-mapping", major(event->dev), minor(event->dev),
		       major(event->keydev), minor(event->keydev), event->agno,
		       event->bno);
		break;
	}
	case EXT4_GET_IMPLIED_CLUSTER_ALLOC_EXIT: {
		if (data_sz < sizeof(ext4_get_implied_cluster_alloc_exit_t))
			return 0;
		ext4_get_implied_cluster_alloc_exit_t *event =
			(typeof(event))data;
		printf("%-14s dev=%u:%u flags=%u lblk=%u pblk=%llu len=%u ret=%d",
		       "-get-implied-cluster-alloc-exit", major(event->dev),
		       minor(event->dev), event->flags, event->lblk,
		       event->pblk, event->len, event->ret);
		break;
	}
	case EXT4_GETFSMAP_HIGH_KEY: {
		...
		printf("%-14s dev=%u:%u keydev=%u:%u block=%llu len=%llu",
		       "-getfsmap-high-key", major(event->dev),
		       minor(event->dev), major(event->keydev),
		       minor(event->keydev), event->block, event->len);
		break;
	}
	case EXT4_GETFSMAP_LOW_KEY: {
		...
		printf("%-14s dev=%u:%u keydev=%u:%u block=%llu len=%llu",
		       "-getfsmap-low-key", major(event->dev),
		       minor(event->dev), major(event->keydev),
		       minor(event->keydev), event->block, event->len);
		break;
	}
	case EXT4_GETFSMAP_MAPPING: {
		...
		printf("%-14s dev=%u:%u keydev=%u:%u block=%llu len=%llu",
		       "-getfsmap-mapping", major(event->dev),
		       minor(event->dev), major(event->keydev),
		       minor(event->keydev), event->block, event->len);
		break;
	}
	case EXT4_IND_MAP_BLOCKS_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ind-map-blocks-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_IND_MAP_BLOCKS_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-ind-map-blocks-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_INSERT_RANGE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-insert-range",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_INVALIDATE_FOLIO: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-invalidate-folio",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_JOURNAL_START_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-journal-start-inode",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_JOURNAL_START_RESERVED: {
		...
		printf("%-14s dev=%u:%u blocks=%d ip=%lx",
		       "-journal-start-reserved", major(event->dev),
		       minor(event->dev), event->blocks, event->ip);
		break;
	}
	case EXT4_JOURNAL_START_SB: {
		...
		printf("%-14s dev=%u:%u blocks=%d ip=%lx type=%d",
		       "-journal-start-sb", major(event->dev),
		       minor(event->dev), event->blocks, event->ip,
		       event->type);
		break;
	}
	case EXT4_JOURNALLED_INVALIDATE_FOLIO: {
		...
		printf("%-14s dev=%u:%u ino=%lu",
		       "-journalled-invalidate-folio", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino);
		break;
	}
	case EXT4_JOURNALLED_WRITE_END: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-journalled-write-end",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_LAZY_ITABLE_INIT: {
		...
		printf("%-14s dev=%u:%u group=%u", "-lazy-itable-init",
	...
	case EXT4_LOAD_INODE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-load-inode",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_LOAD_INODE_BITMAP: {
		...
		printf("%-14s dev=%u:%u group=%u", "-load-inode-bitmap",
		       major(event->dev), minor(event->dev), event->group);
		break;
	}
	case EXT4_MARK_INODE_DIRTY: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mark-inode-dirty",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MB_BITMAP_LOAD: {
		...
		printf("%-14s dev=%u:%u group=%u", "-mb-bitmap-load",
		       major(event->dev), minor(event->dev), event->group);
		break;
	}
	case EXT4_MB_BUDDY_BITMAP_LOAD: {
		...
		printf("%-14s dev=%u:%u group=%u", "-mb-buddy-bitmap-load",
		       major(event->dev), minor(event->dev), event->group);
		break;
	}
	case EXT4_MB_DISCARD_PREALLOCATIONS: {
		...
		printf("%-14s dev=%u:%u needed=%d",
		       "-mb-discard-preallocations", major(event->dev),
		       minor(event->dev), event->needed);
		break;
	}
	case EXT4_MB_NEW_GROUP_PA: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mb-new-group-pa",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MB_NEW_INODE_PA: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mb-new-inode-pa",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MB_RELEASE_GROUP_PA: {
		...
		printf("%-14s dev=%u:%u pa_pstart=%llu pa_len=%u",
		       "-mb-release-group-pa", major(event->dev),
		       minor(event->dev), event->pa_pstart, event->pa_len);
		break;
	}
	case EXT4_MB_RELEASE_INODE_PA: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mb-release-inode-pa",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MBALLOC_ALLOC: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mballoc-alloc",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MBALLOC_DISCARD: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mballoc-discard",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MBALLOC_FREE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mballoc-free",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_MBALLOC_PREALLOC: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-mballoc-prealloc",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_NFS_COMMIT_METADATA: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-nfs-commit-metadata",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_OTHER_INODE_UPDATE_TIME: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-other-inode-update-time",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_PREFETCH_BITMAPS: {
		...
		printf("%-14s dev=%u:%u group=%u next=%u ios=%u",
		       "-prefetch-bitmaps", major(event->dev),
		       minor(event->dev), event->group, event->next,
		       event->ios);
		break;
	}
	case EXT4_PUNCH_HOLE: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-punch-hole",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_READ_BLOCK_BITMAP_LOAD: {
		...
		printf("%-14s dev=%u:%u group=%u prefetch=%s",
		       "-read-block-bitmap-load", major(event->dev),
		       minor(event->dev), event->group,
		       event->prefetch ? "true" : "false");
		break;
	}
	case EXT4_READ_FOLIO: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-read-folio",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_RELEASE_FOLIO: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-release-folio",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_REMOVE_BLOCKS: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-remove-blocks",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_REQUEST_BLOCKS: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-request-blocks",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_REQUEST_INODE: {
		...
		printf("%-14s dev=%u:%u dir=%lu mode=%o", "-request-inode",
		       major(event->dev), minor(event->dev),
	...
	case EXT4_SHUTDOWN: {
		...
		printf("%-14s dev=%u:%u flags=%u", "-shutdown",
		       major(event->dev), minor(event->dev), event->flags);
		break;
	}
	case EXT4_SYNC_FILE_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-sync-file-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_SYNC_FILE_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-sync-file-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_SYNC_FS: {
		...
		printf("%-14s dev=%u:%u wait=%d", "-sync-fs", major(event->dev),
	...
	case EXT4_TRIM_ALL_FREE: {
		...
		printf("%-14s dev=%d:%d group=%u start=%d len=%d",
		       "-trim-all-free", event->dev_major, event->dev_minor,
	...
	case EXT4_TRIM_EXTENT: {
		...
		printf("%-14s dev=%d:%d group=%u start=%d len=%d",
		       "-trim-extent", event->dev_major, event->dev_minor,
		       event->group, event->start, event->len);
		break;
	}
	case EXT4_TRUNCATE_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-truncate-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_TRUNCATE_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-truncate-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_UNLINK_ENTER: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-unlink-enter",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_UNLINK_EXIT: {
		...
		printf("%-14s dev=%u:%u ino=%lu", "-unlink-exit",
		       major(event->dev), minor(event->dev),
		       (unsigned long)event->ino);
		break;
	}
	case EXT4_UPDATE_SB: {
		...
		printf("%-14s dev=%u:%u fsblk=%llu flags=%u", "-update-sb",
		       major(event->dev), minor(event->dev),
	...
	case EXT4_WRITE_BEGIN: {
		...
		printf("%-14s dev=%u:%u ino=%lu pos=%lld len=%u",
		       "-write-begin", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (long long)event->pos,
		       event->len);
		break;
	}
	case EXT4_WRITE_END: {
		...
		printf("%-14s dev=%u:%u ino=%lu pos=%lld len=%u copied=%u",
		       "-write-end", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (long long)event->pos,
		       event->len, event->copied);
		break;
	}
	case EXT4_WRITEPAGES: {
		...
		printf("%-14s dev=%u:%u ino=%lu nr_to_write=%ld pages_skipped=%ld range_start=%lld range_end=%lld sync_mode=%d",
		       "-writepages", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, event->nr_to_write,
		       event->pages_skipped, (long long)event->range_start,
		       (long long)event->range_end, event->sync_mode);
		break;
	}
	case EXT4_WRITEPAGES_RESULT: {
		...
		printf("%-14s dev=%u:%u ino=%lu ret=%d pages_written=%d pages_skipped=%ld sync_mode=%d",
		       "-writepages-result", major(event->dev),
		       minor(event->dev), (unsigned long)event->ino, event->ret,
		       event->pages_written, event->pages_skipped,
		       event->sync_mode);
		break;
	}
	case EXT4_ZERO_RANGE: {
		...
		printf("%-14s dev=%u:%u ino=%lu offset=%lld len=%lld mode=%d",
		       "-zero-range", major(event->dev), minor(event->dev),
		       (unsigned long)event->ino, (long long)event->offset,
		       (long long)event->len, event->mode);
		break;
	}
	default:
