...
const char argp_program_doc[] =
	...
	"\n"
	"USAGE: ext4snoop [-h] [-v] [-t] [-p PID] [-T TID] [-c COMM] [-d DEV] [-D "
	"DURATION]\n"
	"\n"
	"EXAMPLES:\n"
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	case EXT4_ALLOC_DA_BLOCKS:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu data_blocks=%u",
			"-alloc-da-blocks",
			major(event->dev),
			minor(event->dev),
	...
	case EXT4_ALLOCATE_BLOCKS:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu block=%llu len=%u logical=%u lleft=%u "
			"lright=%u goal=%llu pleft=%llu pright=%llu flags=%u",
			"-allocate-blocks",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
			(unsigned long long)event->block,
			event->len,
			event->logical,
			event->lleft,
			event->lright,
			...
			(unsigned long long)event->pright,
			event->flags
		);
		break;
	}
	case EXT4_ALLOCATE_INODE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu dir=%lu mode=%o",
			"-allocate-inode",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
			(unsigned long)event->dir,
			event->mode
		);
		break;
	}
	case EXT4_BEGIN_ORDERED_TRUNCATE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu new_size=%lld",
	...
	case EXT4_COLLAPSE_RANGE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu offset=%lld len=%lld",
			"-collapse-range",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
			(long long)event->offset,
			(long long)event->len
		);
		break;
	}
	case EXT4_DA_RELEASE_SPACE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu i_blocks=%llu freed_blocks=%d "
			"reserved_data_blocks=%d mode=%o",
			"-da-release-space",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
	...
	case EXT4_DA_RESERVE_SPACE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu i_blocks=%llu reserved_data_blocks=%d "
			"mode=%o",
			"-da-reserve-space",
			major(event->dev),
			minor(event->dev),
	...
	case EXT4_DA_UPDATE_RESERVE_SPACE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu i_blocks=%llu used_blocks=%d "
			"reserved_data_blocks=%d quota_claim=%d mode=%o",
	...
	case EXT4_DA_WRITE_BEGIN:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu pos=%lld len=%u",
			"-da-write-begin",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
	...
	case EXT4_DA_WRITE_END:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu pos=%lld len=%u copied=%u",
			"-da-write-end",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
	...
	case EXT4_DA_WRITE_PAGES:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu first_page=%lu nr_to_write=%ld "
			"sync_mode=%d",
			"-da-write-pages",
			major(event->dev),
			minor(event->dev),
	...
	case EXT4_DA_WRITE_PAGES_EXTENT:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu lblk=%llu len=%u flags=%u",
	...
	case EXT4_DISCARD_PREALLOCATIONS:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu len=%u needed=%u",
	...
	case EXT4_ES_CACHE_EXTENT:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu lblk=%u len=%u pblk=%llu status=%c",
			"-es-cache-extent",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
	...
	case EXT4_ES_SHRINK:
	{
		...
		printf(
			"%-14s dev=%u:%u nr_shrunk=%d scan_time=%llu",
			"-es-shrink",
			major(event->dev),
			minor(event->dev),
			event->nr_shrunk,
	...
	case EXT4_ES_SHRINK_COUNT:
	{
		...
		printf(
			"%-14s dev=%u:%u nr_to_scan=%d cache_cnt=%d",
			"-es-shrink-count",
			major(event->dev),
			minor(event->dev),
			event->nr_to_scan,
	...
	case EXT4_ES_SHRINK_SCAN_EXIT:
	{
		...
		printf(
			"%-14s dev=%u:%u nr_shrunk=%d cache_cnt=%d",
			"-es-shrink-scan-exit",
			major(event->dev),
			minor(event->dev),
			event->nr_shrunk,
	...
	case EXT4_FC_CLEANUP:
	{
		...
		printf(
			"%-14s dev=%u:%u j_fc_off=%d full=%d tid=%u",
			"-fc-cleanup",
			major(event->dev),
			minor(event->dev),
			event->j_fc_off,
	...
	case EXT4_FC_COMMIT_STOP:
	{
		...
		printf(
			"%-14s dev=%u:%u nblks=%d reason=%d tid=%u",
			"-fc-commit-stop",
			major(event->dev),
			minor(event->dev),
			event->nblks,
	...
	case EXT4_FC_STATS:
	{
		...
		printf(
			"%-14s dev=%u:%u fc_commits=%lu fc_ineligible_commits=%lu",
			"-fc-stats",
			major(event->dev),
			minor(event->dev),
			event->fc_commits,
			event->fc_ineligible_commits
		);
		break;
	}
	case EXT4_FC_TRACK_CREATE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu tid=%u",
			"-fc-track-create",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->i_ino,
			event->t_tid
		);
		break;
	}
	...
	case EXT4_FC_TRACK_RANGE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu tid=%u start=%ld end=%ld",
			"-fc-track-range",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->i_ino,
	...
	case EXT4_FSMAP_HIGH_KEY:
	{
		...
		printf(
			"%-14s dev=%u:%u keydev=%u:%u agno=%u bno=%llu",
			"-fsmap-high-key",
			major(event->dev),
			minor(event->dev),
			major(event->keydev),
	...
	case EXT4_GET_IMPLIED_CLUSTER_ALLOC_EXIT:
	{
		...
		printf(
			"%-14s dev=%u:%u flags=%u lblk=%u pblk=%llu len=%u ret=%d",
	...
	case EXT4_GETFSMAP_HIGH_KEY:
	{
		...
		printf(
			"%-14s dev=%u:%u keydev=%u:%u block=%llu len=%llu",
			"-getfsmap-high-key",
			major(event->dev),
			minor(event->dev),
			major(event->keydev),
	...
	case EXT4_JOURNAL_START_SB:
	{
		...
		printf(
			"%-14s dev=%u:%u blocks=%d ip=%lx type=%d",
			"-journal-start-sb",
			major(event->dev),
			minor(event->dev),
			event->blocks,
	...
	case EXT4_MB_RELEASE_GROUP_PA:
	{
		...
		printf(
			"%-14s dev=%u:%u pa_pstart=%llu pa_len=%u",
			"-mb-release-group-pa",
			major(event->dev),
			minor(event->dev),
			event->pa_pstart,
	...
	case EXT4_PREFETCH_BITMAPS:
	{
		...
		printf(
			"%-14s dev=%u:%u group=%u next=%u ios=%u",
			"-prefetch-bitmaps",
			major(event->dev),
			minor(event->dev),
			event->group,
	...
	case EXT4_READ_BLOCK_BITMAP_LOAD:
	{
		...
		printf(
			"%-14s dev=%u:%u group=%u prefetch=%s",
	...
	case EXT4_TRIM_ALL_FREE:
	{
		...
		printf(
			"%-14s dev=%d:%d group=%u start=%d len=%d",
			"-trim-all-free",
			event->dev_major,
			event->dev_minor,
			event->group,
	...
	case EXT4_UPDATE_SB:
	{
		...
		printf(
			"%-14s dev=%u:%u fsblk=%llu flags=%u",
			"-update-sb",
			major(event->dev),
			minor(event->dev),
			(unsigned long long)event->fsblk,
			event->flags
		);
		break;
	}
	...
	case EXT4_WRITEPAGES:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu nr_to_write=%ld pages_skipped=%ld "
			...
			event->nr_to_write,
			event->pages_skipped,
			(long long)event->range_start,
			(long long)event->range_end,
			event->sync_mode
		);
		break;
	}
	case EXT4_WRITEPAGES_RESULT:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu ret=%d pages_written=%d pages_skipped=%ld "
			"sync_mode=%d",
			"-writepages-result",
			major(event->dev),
			minor(event->dev),
	...
	case EXT4_ZERO_RANGE:
	{
		...
		printf(
			"%-14s dev=%u:%u ino=%lu offset=%lld len=%lld mode=%d",
			"-zero-range",
			major(event->dev),
			minor(event->dev),
			(unsigned long)event->ino,
