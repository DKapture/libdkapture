...
static void print_extent_writepage_event(const struct btrfs_extent_writepage_event *e)
{
    ...
    printf("%-16s %-7d %-7d %-30s ino=%llu index=%lu nr_to_write=%ld "
           ...
           e->ino, e->index, e->nr_to_write,
           e->range_start, e->range_end, e->root_objectid,
           format_fsid(e->fsid).c_str());
}

static void print_add_delayed_data_ref_event(const struct btrfs_add_delayed_data_ref_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
    ...
    printf("%-16s %-7d %-7d %-30s bytenr=%llu bytes=%llu action=%d "
           ...
           e->base.comm, e->base.pid, e->base.tid, "btrfs_add_delayed_data_ref",
           e->bytenr, e->num_bytes, e->action, e->parent,
           e->ref_root, e->owner, e->offset, e->type);
}

static void print_add_delayed_ref_head_event(const struct btrfs_add_delayed_ref_head_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
    ...
    printf("%-16s %-7d %-7d %-30s bytenr=%llu bytes=%llu action=%d is_data=%d\n",
           e->base.comm, e->base.pid, e->base.tid, "btrfs_add_delayed_ref_head",
           e->bytenr, e->num_bytes, e->action, e->is_data);
}
...
static void print_transaction_commit_event(const struct btrfs_transaction_commit_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
    ...
    printf("%-16s %-7d %-7d %-30s generation=%llu root=%llu\n",
           e->base.comm, e->base.pid, e->base.tid, "btrfs_transaction_commit",
           e->generation, e->root_objectid);
}

static void print_space_reservation_event(const struct btrfs_space_reservation_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
    ...
    printf("%-16s %-7d %-7d %-30s bytes=%llu reserve=%d\n",
           e->base.comm, e->base.pid, e->base.tid, "btrfs_space_reservation",
           e->bytes, e->reserve);
}

static void print_ordered_extent_add_event(const struct btrfs_ordered_extent_add_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
    ...
    printf("%-16s %-7d %-7d %-30s ino=%llu start=%llu len=%llu flags=0x%lx\n",
           e->base.comm, e->base.pid, e->base.tid, "btrfs_ordered_extent_add",
           e->ino, e->start, e->len, e->flags);
}
...
static void print_qgroup_account_extent_event(const struct btrfs_qgroup_account_extent_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
    ...
    printf("%-16s %-7d %-7d %-30s bytenr=%llu bytes=%llu old_roots=%llu new_roots=%llu\n",
           e->base.comm, e->base.pid, e->base.tid, "btrfs_qgroup_account_extent",
           e->bytenr, e->num_bytes, e->nr_old_roots, e->nr_new_roots);
}
...
static void print_reserve_extent_event(const struct btrfs_reserve_extent_event *e)
{
    if (should_filter_event(e->base.pid, e->base.tid, e->base.comm))
        return;
...
static void print_default_event(const struct btrfs_base_event *e, const char *event_name)
{
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    case BTRFS_EXTENT_WRITEPAGE:
        print_extent_writepage_event(static_cast<struct btrfs_extent_writepage_event *>(data));
        break;
    case BTRFS_ADD_DELAYED_DATA_REF:
        print_add_delayed_data_ref_event(static_cast<struct btrfs_add_delayed_data_ref_event *>(data));
        break;
    case BTRFS_ADD_DELAYED_REF_HEAD:
        print_add_delayed_ref_head_event(static_cast<struct btrfs_add_delayed_ref_head_event *>(data));
        break;
    case BTRFS_CHUNK_ALLOC:
        print_chunk_alloc_event(static_cast<struct btrfs_chunk_alloc_event *>(data));
        break;
    case BTRFS_CHUNK_FREE:
        print_chunk_free_event(static_cast<struct btrfs_chunk_free_event *>(data));
        break;
    case BTRFS_TRANSACTION_COMMIT:
        print_transaction_commit_event(static_cast<struct btrfs_transaction_commit_event *>(data));
        break;
    case BTRFS_SPACE_RESERVATION:
        print_space_reservation_event(static_cast<struct btrfs_space_reservation_event *>(data));
        break;
    case BTRFS_ORDERED_EXTENT_ADD:
        print_ordered_extent_add_event(static_cast<struct btrfs_ordered_extent_add_event *>(data));
        break;
    case BTRFS_SYNC_FILE:
        print_sync_file_event(static_cast<struct btrfs_sync_file_event *>(data));
        break;
    case BTRFS_QGROUP_ACCOUNT_EXTENT:
        print_qgroup_account_extent_event(static_cast<struct btrfs_qgroup_account_extent_event *>(data));
        break;
    ...
    case BTRFS_RESERVE_EXTENT:
        print_reserve_extent_event(static_cast<struct btrfs_reserve_extent_event *>(data));
        break;
