    ...
    void *ctx)
{
    ...
    if (rule->remote_ip &&
        rule->remote_ip != log->remote_ip)
    {
        return 0;
    }

    if (rule->remote_port &&
        rule->remote_port != log->remote_port)
    {
        return 0;
    }
...
static int traffic_stat(struct socket *sock, int ret, int dir)
{
    ...
    bpf_read_kmem_ret(&skc_rcv_saddr,
        &sk->__sk_common.skc_rcv_saddr,
        return 0);

    bpf_read_kmem_ret(&skc_dport,
    ...
    if (dst_addr == IP(127, 0, 0, 1) ||
        src_addr == IP(127, 0, 0, 1))
        return 0;
...
int BPF_PROG(__sys_sendto_exit,
