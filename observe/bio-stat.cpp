...
static void snoop(void)
{
	...
	while (!exit_flag)
	{
		...
		if (err < 0 && err != -EINTR)
		{
			fprintf(
				stderr,
				"pr_error polling perf buffer: %s\n",
				strerror(-err)
			);
			break;
		}
...
static void top(void)
{
	...
	while (!exit_flag)
	{
		...
		for (u32 i = 0; i < count; i++)
		{
			...
			printf(
				"%-7d %-16s %1s %3d:%-3d %-8s %5u %7llu %6.2f\n",
				k.pid,
				k.name,
				k.rwflag ? "W" : "R",
				k.major,
