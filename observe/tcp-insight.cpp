...
static const char* tcp_event_type_str(int type) {
    switch (type) {
        case TCP_EVENT_STATE_CHANGE: return "STATE_CHANGE";
        case TCP_EVENT_RETRANSMIT: return "RETRANSMIT";
        case TCP_EVENT_SEND_RESET: return "SEND_RESET";
        case TCP_EVENT_RECV_RESET: return "RECV_RESET";
        case TCP_EVENT_SEND_DATA: return "SEND_DATA";
        case TCP_EVENT_RECV_DATA: return "RECV_DATA";
        case TCP_EVENT_SOCK_DESTROY: return "DESTROY";
        case TCP_EVENT_CONG_STATE: return "CONG_STATE";
        case TCP_EVENT_WIN_ADJUST: return "WIN_ADJUST";
        case TCP_EVENT_PERF_SAMPLE: return "PERF_SAMPLE";
        case TCP_EVENT_KPROBE_SEND: return "KPROBE_SEND";
        case TCP_EVENT_KPROBE_RECV: return "KPROBE_RECV";
        case TCP_EVENT_KPROBE_RETRANS: return "KPROBE_RETRANS";
    ...
    switch (key) {
        ...
        case 'S':
            ...
            if (env.state_filter < 1 || env.state_filter >= TCP_MAX_STATES) {
                fprintf(stderr, "Invalid TCP state: %s (valid range: 1-%d)\n", 
...
static int setup_filters() {
    ...
    if (verbose) {
        ...
        if (env.pid > 0) printf("  PID: %d\n", env.pid);
        if (strlen(env.comm) > 0) printf("  Command: %s\n", env.comm);
        if (strlen(env.saddr) > 0) printf("  Source address: %s\n", env.saddr);
        if (strlen(env.daddr) > 0) printf("  Destination address: %s\n", env.daddr);
        if (env.sport > 0) printf("  Source port: %d\n", env.sport);
        if (env.dport > 0) printf("  Destination port: %d\n", env.dport);
        if (env.event_mask > 0) printf("  Event mask: 0x%x\n", env.event_mask);
        if (env.min_duration > 0) printf("  Min duration: %d ms\n", env.min_duration);
        if (env.max_duration > 0) printf("  Max duration: %d ms\n", env.max_duration);
        if (env.min_bytes > 0) printf("  Min bytes: %ld\n", env.min_bytes);
        if (env.max_bytes > 0) printf("  Max bytes: %ld\n", env.max_bytes);
        if (env.min_rtt > 0) printf("  Min RTT: %d us\n", env.min_rtt);
        if (env.max_rtt > 0) printf("  Max RTT: %d us\n", env.max_rtt);
        if (env.state_filter > 0) printf("  TCP state: %s\n", tcp_state_str(env.state_filter));
        printf("\n");
    }
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    ...
    // Format addresses
    format_address(saddr_buf, sizeof(saddr_buf), ((e->family == AF_INET) ? e->addr.ipv4.saddr : 0), e->sport);
    format_address(daddr_buf, sizeof(daddr_buf), ((e->family == AF_INET) ? e->addr.ipv4.daddr : 0), e->dport);
    ...
    switch (e->type) {
        case TCP_EVENT_STATE_CHANGE:
            ...
            snprintf(details_buf, sizeof(details_buf), "skaddr=0x%p", e->data.state_change.skaddr);
            break;
            
        case TCP_EVENT_PERF_SAMPLE:
            snprintf(cwnd_buf, sizeof(cwnd_buf), "%u", e->data.perf_sample.snd_cwnd);
            snprintf(rtt_buf, sizeof(rtt_buf), "%u", e->data.perf_sample.srtt);
            snprintf(bytes_buf, sizeof(bytes_buf), "%u", e->data.perf_sample.data_len);
            snprintf(details_buf, sizeof(details_buf), "ssthresh=%u rcv_wnd=%u", 
                    e->data.perf_sample.ssthresh, e->data.perf_sample.rcv_wnd);
            break;
            
        case TCP_EVENT_RETRANSMIT:
            snprintf(state_buf, sizeof(state_buf), "%s", tcp_state_str(e->data.retransmit.state));
        ...
        case TCP_EVENT_SEND_RESET:
        case TCP_EVENT_RECV_RESET:
            snprintf(state_buf, sizeof(state_buf), "%s", tcp_state_str(e->data.reset.state));
            snprintf(details_buf, sizeof(details_buf), "skb=0x%p sk=0x%p cookie=%llu", 
                    e->data.reset.skbaddr, e->data.reset.skaddr, e->data.reset.sock_cookie);
            break;
            
        case TCP_EVENT_CONG_STATE:
            snprintf(details_buf, sizeof(details_buf), "cong_state=%u sk=0x%p", 
                    e->data.cong_state.cong_state, e->data.cong_state.skaddr);
            break;
            
        case TCP_EVENT_WIN_ADJUST:
            snprintf(details_buf, sizeof(details_buf), "sk=0x%p cookie=%llu", 
                    e->data.win_adjust.skaddr, e->data.win_adjust.sock_cookie);
            break;
            
        case TCP_EVENT_SOCK_DESTROY:
            snprintf(details_buf, sizeof(details_buf), "sk=0x%p cookie=%llu", 
                    e->data.destroy.skaddr, e->data.destroy.sock_cookie);
            break;
    ...
    printf("%-16s %-6d %-6d %-15s %-22s %-22s %-12s %-6s %-6s %-8s %s\n",
           ...
           saddr_buf, daddr_buf,
           state_buf, cwnd_buf, rtt_buf, bytes_buf, details_buf);
