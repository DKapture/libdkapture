...
// 辅助函数：应用过滤规则
static bool should_trace(struct net_rule *rule, u32 pid, u32 len, const char *dev_name)
{
...
static void parse_ip_header(struct sk_buff *skb, struct net_event *event)
{
    ...
    if (bpf_probe_read(&ip_hdr, sizeof(ip_hdr), 
                      (void *)(long)BPF_CORE_READ(skb, head) + network_offset) != 0)
        return;
...
static void parse_tcp_header(struct sk_buff *skb, struct net_event *event)
{
    ...
    if (bpf_probe_read(&tcp_hdr, sizeof(tcp_hdr), 
                      (void *)(long)BPF_CORE_READ(skb, head) + transport_offset) != 0)
        return;
    ...
    // 解析TCP标志位
    event->tcp_flags = 0;
    if (tcp_hdr.syn) event->tcp_flags |= TCP_FLAG_SYN;
    if (tcp_hdr.ack) event->tcp_flags |= TCP_FLAG_ACK;
    if (tcp_hdr.fin) event->tcp_flags |= TCP_FLAG_FIN;
    if (tcp_hdr.rst) event->tcp_flags |= TCP_FLAG_RST;
}
...
int trace_netif_receive_skb(struct tp_netif_receive_skb *ctx)
{
    ...
    if (!should_trace(rule, bpf_get_current_pid_tgid() >> 32, ctx->len, dev_name))
        return 0;
