...
int main(int argc, char **argv)
{
    ...
    if (env.utilization_only && env.stall_only) {
        fprintf(stderr, "Error: --utilization-only and --stall-only are mutually exclusive\n");
        return 1;
    }
    ...
    // 设置过滤规则
    filter.enabled = (env.pid || env.cpu != -1 || env.utilization_only || env.stall_only);
    ...
    err = bpf_map_update_elem(bpf_map__fd(skel->maps.filter_map), &key, &filter, 0);
    if (err) {
    ...
    while (!exiting) {
        ...
        // 检查是否到达运行时间限制
        if (env.times != 99999999 && (long)(time(NULL) - stats.start_time) >= env.times) {
            break;
        }
    }
