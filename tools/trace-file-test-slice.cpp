...
void copy_file(const char *src_filename, const char *dst_filename) {
    ...
    while ((bytes_copied = splice(src_fd, nullptr, pipe_fd[1], nullptr, 4096, 0)) > 0) {
        if (splice(pipe_fd[0], nullptr, dst_fd, nullptr, bytes_copied, 0) == -1) {
            std::cerr << "Failed to splice to destination file: " << strerror(errno) << std::endl;
            close(src_fd);
            close(dst_fd);
            close(pipe_fd[0]);
            close(pipe_fd[1]);
    ...
    if (bytes_copied == -1) {
        std::cerr << "Failed to splice from source file: " << strerror(errno) << std::endl;
    } else {
        std::cout << "File copied successfully from " << src_filename << " to " << dst_filename << std::endl;
    }
