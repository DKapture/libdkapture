...
void cleanup_and_exit(int sig)
{
	...
	if (udp_sender_pid > 0)
	{
		...
		{
			log_info(
				"已发送SIGTERM到UDP发送器 (PID: " +
				std::to_string(udp_sender_pid) + ")"
			);
		}
		else
		{
			log_warning(
				"发送SIGTERM到UDP发送器失败: " + std::string(strerror(errno))
			);
		}
	}
	...
	if (udp_receiver_pid > 0)
	{
		...
		{
			log_info(
				"已发送SIGTERM到UDP接收器 (PID: " +
				std::to_string(udp_receiver_pid) + ")"
			);
		}
		else
		{
			log_warning(
				"发送SIGTERM到UDP接收器失败: " + std::string(strerror(errno))
			);
		}
	}
	...
	if (tc_if_pid > 0)
	{
		...
		{
			log_info(
				"已发送SIGTERM到tc-if (PID: " + std::to_string(tc_if_pid) + ")"
			);
		}
		else
		{
			log_warning(
				"发送SIGTERM到tc-if失败: " + std::string(strerror(errno))
			);
		}
	}
	...
	if (shared_stats != nullptr)
	{
		...
		{
			log_warning(
				"共享内存映射清理失败: " + std::string(strerror(errno))
			);
		}
...
bool validate_system_requirements(void)
{
	...
	{
		log_error(
			"tc-if程序未找到或不可执行: " + std::string(TC_IF_PATH),
			ERROR_TC_IF_NOT_FOUND
		);
		return false;
	}
	...
	if (getrlimit(RLIMIT_MEMLOCK, &rlim) == 0)
	{
		...
		{
			log_warning(
				"RLIMIT_MEMLOCK较小 (" + std::to_string(rlim.rlim_cur) +
				")，可能影响BPF程序加载"
			);
		}
		else
		{
			log_success(
				"RLIMIT_MEMLOCK检查通过 (" + std::to_string(rlim.rlim_cur) + ")"
			);
		}
	}
	else
...
/*
 * 如果想测试tc-ip, tc-process,
 * tc-cgroup等程序，需要在构建启动命令这里加入要执行的命令，默认测试限速是1M/s.
 * 目前只有tc-if程序的限速功能测试
 *
 */
...
{
	log_info(
		"启动tc-if进程，方向: " + direction +
		"，限速: " + std::to_string(rate_mbps) + " MB/s"
	);
	 ...
	 */
	std::string tc_cmd = std::string(TC_IF_PATH) + " -I lo -r " + rate_str +
						 " -d " + direction + " -t 1 &";
	...
	{
		log_error(
			"tc-if程序启动失败，返回码: " + std::to_string(result),
			result
		);
		return false;
	}
...
{
	log_info(
		"[PID: " + std::to_string(tcp_current_pid) + "] 收到信号 " +
		std::to_string(sig) + ", 正在退出..."
	);
	tcp_running = 0;
}
...
int tcp_server_main()
{
	...
	log_info(
		"客户端已连接: " + std::string(inet_ntoa(client_addr.sin_addr)) + ":" +
	...
	while (tcp_running)
	{
		...
		if (current_time != last_report_time)
		{
			...
			log_info(
				"时间: " + std::to_string(elapsed / 60) + ":" +
				std::to_string(elapsed % 60) +
		...
		else
		{
			// 正常模式：随机间隔，根据速率倍数调整
			delay_us =
				(int)(generate_send_interval() / current_rate_multiplier);
			burst_mode = 0; // 退出突发模式
		}
...
int tcp_client_main()
{
	...
	log_info("TCP客户端启动");
	log_info(
		"连接到服务器 " + std::string(SERVER_IP) + ":" +
		std::to_string(TCP_CLIENT_PORT)
	);
...
int udp_receiver_main(void)
{
	...
	SharedStats *stats = (SharedStats *)mmap(
		NULL,
		sizeof(SharedStats),
		PROT_READ | PROT_WRITE,
		MAP_SHARED,
	...
	{
		log_info(
			"接收缓冲区大小设置为: " + std::to_string(rcvbuf_size) + " 字节"
		);
	}
	...
	{
		log_info(
			"成功绑定到端口: " + std::to_string(ntohs(server_addr.sin_port))
		);
	}
	else
	{
	...
	while (true)
	{
		...
		if (received > 0)
		{
			...
			if (current_time != last_report_time)
			{
				...
				double total_rate_mibps =
					(stats->total_bytes_received / (1024.0 * 1024.0)) /
					(elapsed + 1);
				...
				log_info(
					...
					"    带宽: " + std::to_string(rate_mibps) +
					" MiB/s (累计: " + std::to_string(total_rate_mibps) +
					" MiB/s)"
				);
		...
		else if (received < 0)
		{
			...
			{
				log_error(
					"recvfrom失败: " + std::string(strerror(errno)),
					errno
				);
				perror("recvfrom失败");
	...
	if (stats != nullptr)
	{
		...
		if (total_time > 0)
		{
			stats->avg_ingress_rate =
				(stats->ingress_bytes / (1024.0 * 1024.0)) /
				(INGRESS_TEST_DURATION > 0 ? INGRESS_TEST_DURATION : 1);
			stats->avg_egress_rate =
				(stats->egress_bytes / (1024.0 * 1024.0)) /
				((TEST_DURATION_SEC - INGRESS_TEST_DURATION) > 0
					 ? (TEST_DURATION_SEC - INGRESS_TEST_DURATION)
					 : 1);
		}
	}
...
int generate_send_interval(double target_rate_mbps)
{
	...
	double packets_per_second =
		(target_rate_mbps * 1024 * 1024) / avg_packet_size;
...
int udp_sender_main(void)
{
	...
	log_info("开始发送UDP数据包...");
	log_info(
		"目标地址: " + local_ip_address + ":" + std::to_string(LOCAL_PORT)
	);
	...
	{
		log_info(
			"发送缓冲区大小设置为: " + std::to_string(sndbuf_size) + " 字节"
		);
	}
	...
	// 使用动态获取的IP地址
	if (inet_pton(AF_INET, local_ip_address.c_str(), &server_addr.sin_addr) <=
		0)
	{
		log_error(
			"无效的目标IP地址: " + local_ip_address,
	...
	log_info(
		"已设置目标地址: " + local_ip_address + ":" + std::to_string(LOCAL_PORT)
	);
	...
	while (true)
	{
		...
		if (current_time != last_rate_change)
		{
			if (current_target_rate >= 3.0 &&
				high_load_duration < max_high_load_duration)
			{
				// 高负载持续时间控制 - 避免长时间高负载
				high_load_duration++;
				current_target_rate = 3.0 + (rand() % 500) / 100.0;
				log_info(
					"[PID: " + std::to_string(tcp_current_pid) +
					"] 高负载模式 #" + std::to_string(high_load_duration) +
					"，目标速率: " + std::to_string(current_target_rate) +
					" MB/s"
				);
			}
			else
		...
		if (sent > 0)
		{
			...
			if (current_time != last_report_time)
			{
				...
				{
					avg_send_rate =
						(total_bytes_sent / (1024.0 * 1024.0)) / elapsed;
				}

				log_info(
					"[PID: " + std::to_string(current_pid) + "] " +
					std::string(elapsed / 60 < 10 ? "0" : "") +
					std::to_string(elapsed / 60) + ":" +
					std::string(elapsed % 60 < 10 ? "0" : "") +
					std::to_string(elapsed % 60) + "    " +
					std::to_string(packets_sent) + "    " +
					std::to_string(bytes_sent) + "    " +
					std::to_string(rate_mibps) + "    " +
					std::to_string(current_target_rate)
				);
		...
		else if (sent < 0)
		{
			...
			else
			{
				log_error(
					"[PID: " + std::to_string(current_pid) +
						"] sendto失败: " + std::string(strerror(errno)),
					errno
				);
				perror("sendto失败");
	...
	log_info(
		"[PID: " + std::to_string(current_pid) +
		"]   - 总发送字节: " + std::to_string(total_bytes_sent)
	);
	log_info(
		"[PID: " + std::to_string(current_pid) +
		"]   - 总发送包数: " + std::to_string(total_packets_sent)
	);
	log_info(
		"[PID: " + std::to_string(current_pid) +
		"]   - 峰值发送速率: " + std::to_string(peak_send_rate) + " MB/s"
	);
	log_info(
		"[PID: " + std::to_string(current_pid) +
		"]   - 平均发送速率: " + std::to_string(avg_send_rate) + " MB/s"
	);
...
int main(void)
{
	...
	log_info(
		"  - Ingress测试时长: " + std::to_string(INGRESS_TEST_DURATION) + " 秒"
	);
	log_info(
		"  - Egress测试时长: " + std::to_string(EGRESS_TEST_DURATION) + " 秒"
	);
	log_info("  - 目标限速: " + std::to_string(TARGET_RATE_MBPS) + " MB/s");
	log_info(
		"  - 最大允许速率: " + std::to_string(MAX_ALLOWED_RATE_MBPS) + " MB/s"
	);
	...
	log_success(
		"UDP接收器已启动 (PID: " + std::to_string(udp_receiver_pid) + ")"
	);
	...
	log_success(
		"UDP发送器已启动 (PID: " + std::to_string(udp_sender_pid) + ")"
	);
	...
	log_info(
		"🔴 第一阶段：接收限速测试 (0-" +
		std::to_string(INGRESS_TEST_DURATION) + "秒)"
	);
	...
	// 第二阶段：发送限速测试 (egress)
	log_info(
		"🔵 第二阶段：发送限速测试 (" + std::to_string(INGRESS_TEST_DURATION) +
		"-" + std::to_string(TEST_DURATION_SEC) + "秒)"
	);
	...
	{
		std::string status =
			(kill(udp_sender_pid, 0) == 0) ? "运行中" : "已退出";
		log_info(
			"UDP发送器PID: " + std::to_string(udp_sender_pid) +
			" (状态: " + status + ")"
		);
	}
	...
	{
		std::string status =
			(kill(udp_receiver_pid, 0) == 0) ? "运行中" : "已退出";
		log_info(
			"UDP接收器PID: " + std::to_string(udp_receiver_pid) +
			" (状态: " + status + ")"
		);
	}
	...
	{
		log_info(
			"总接收字节: " + std::to_string(shared_stats->total_bytes_received)
		);
		log_info(
			"总发送字节: " + std::to_string(shared_stats->total_bytes_sent)
		);
	...
	// 计算各阶段平均速率
	double ingress_rate =
		(ingress_bytes / (1024.0 * 1024.0)) / INGRESS_TEST_DURATION;
	double egress_test_duration = EGRESS_TEST_DURATION;
	double egress_rate =
		(egress_bytes / (1024.0 * 1024.0)) / egress_test_duration;
	...
	log_info(
		"  - 峰值速率: " + std::to_string(shared_stats->peak_ingress_rate) +
		" MB/s"
	);
	...
	log_info(
		"  - 实际测试时间: " + std::to_string(egress_test_duration) + " 秒"
	);
	...
	log_info(
		"  - 峰值速率: " + std::to_string(shared_stats->peak_egress_rate) +
		" MB/s"
	);
	...
	log_info(
		"说明：此阶段tc-if在egress方向限速，接收器接收到的流量应该被限制在" +
		std::to_string(TARGET_RATE_MBPS) + " MB/s左右"
	);
