...
void copy_file(const char *src_filename, const char *dst_filename) {
    ...
    while ((bytes_copied = copy_file_range(src_fd, &src_offset, dst_fd, &dst_offset, len, 0)) > 0) {
        std::cout << "Copied " << bytes_copied << " bytes from " << src_filename << " to " << dst_filename << std::endl;
    }
