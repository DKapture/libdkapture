...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 93) && LINUX_VERSION_CODE < KERNEL_VERSION(6, 9, 6)
static struct file *(*pf_uld_check_file)(struct fdtable *fdt, struct file *file, unsigned fd);
struct file *uld_fcheck_file(struct fdtable *fdt, struct file *file, unsigned fd)
{
...
static struct file *find_task_path(
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 93) && LINUX_VERSION_CODE < KERNEL_VERSION(6, 9, 6)
    pf_uld_check_file = (typeof(pf_uld_check_file))lookup_symbol("uld_check_file");
...
MODULE_DESCRIPTION("a module helping to retreat accidentally deleted files(if it's still referenced by any process)");