...
public:
    ...
    virtual ssize_t read(std::vector<DataType> &dts, pid_t pid, DataHdr *buf, size_t bsz);
    virtual ssize_t read(std::vector<const char *> &paths, DataHdr *buf, size_t bsz);
    virtual ssize_t read(DataType dt, std::vector<pid_t> &pids, DataHdr *buf, size_t bsz);
    ...
    virtual ssize_t read(std::vector<DataType> &dts, DKCallback cb, void *ctx) override;
    virtual ssize_t read(std::vector<const char *> &paths, DKCallback cb, void *ctx) override;
    virtual int kmemleak_scan_start(pid_t pid, DKCallback cb, void *ctx) override;
    virtual int kmemleak_scan_stop(void) override;
    virtual int file_watch(
        const char *path, DKCallback cb, void *ctx) override;
...
ssize_t dkapture::read(std::vector<const char *> &paths, DKCallback cb, void *ctx)
{
    ssize_t total = 0;
    for (auto path : paths)
    {
...
int trace_file_init(int argc, char **argv, int (*cb)(void *, const void *, size_t), void *ctx);
int dkapture::file_watch(
    const char *path, DKCallback cb, void *ctx)
{
    if (cb == nullptr)
