...
int DataMap::sub_iterator(ulong idx, void *buf, size_t bsz) const
{
	...
	for (ulong i = st; i < idx; i++)
	{
		...
		DEBUG(0, "sub_iterator i: %lu idx: %lu hash: %lx", i,
		      m_entrys[i].data_idx, m_entrys[i].hash);
		if (buf)
		{
			if (bsz < dh->dsz)
			{
				pr_error(
					"dkapture::read: buffer size is too small, %lu needed, %lu provided",
					dh->dsz, bsz);
				return -ENOBUFS;
			}
...
int DataMap::unsafe_find(ulong hash, ulong lifetime, void *buf, size_t bsz)
{
	...
	for (long i = eidx; i >= sidx; i--)
	{
		if (m_entrys[i].hash)
		{
			DEBUG(0, "i: %lu rb-idx: %lu hash: %lx vs %lx", i,
			      m_entrys[i].data_idx, m_entrys[i].hash, hash);
		}
		/**
		...
		DEBUG(0, "time(%lu) cmp: %lu vs %lu", lifetime,
		      m_entrys[i].time + lifetime, now);
		...
		{
			pr_error(
				"dkapture::read: buffer size is too small, %lu needed, %lu privided",
				dh->dsz, bsz);
			return -ENOBUFS;
		}
...
void DataMap::list_all_entrys(void)
{
	...
	for (long i = sidx; i < eidx; i++)
	{
		...
		pr_info("idx: %lu, data_idx: %lu, hash: %lx, time: %lu", i,
			ent.data_idx, ent.hash, ent.time);
	}
}