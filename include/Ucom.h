...
#define bpf_syscall(cmd, attr)                             \
    ({                                                     \
        ...
        if (fd < 0)                                        \
        {                                                  \
            pr_error("bpf syscall failure(%s): cmd: " #cmd \
                     " line: %d",                          \
...
#define bpf_attach_kprobe(prog, probe)                         \
    ({                                                         \
        ...
        if (!link)                                             \
        {                                                      \
            pr_error("bpf attach kprobe failure(%s)"           \
                     " line: %d",                              \
                     strerror(errno), __LINE__);               \
            goto err_out;                                      \
        }                                                      \
...
#define bpf_attach_kretprobe(prog, probe)                     \
    ({                                                        \
        ...
        if (!link)                                            \
        {                                                     \
            pr_error("bpf attach kretprobe failure(%s)"       \
                     " line: %d",                             \
                     strerror(errno), __LINE__);              \
            goto err_out;                                     \
        }                                                     \
...
#define bpf_create_iter(link, opt)                       \
    ({                                                   \
        ...
        if (ifd < 0)                                     \
        {                                                \
            pr_error("fail to creating iterator '" #link \
                     "': %s",                            \
                     strerror(errno));                   \
            opt;                                         \
        }                                                \
...
public:
    ...
    void follow(FILE *fp = nullptr)
    {
        ...
        tracing_pipe_fd = open("/sys/kernel/debug/tracing/trace_pipe", O_RDONLY);
        if (tracing_pipe_fd < 0)
        {
    ...
    void async_follow(FILE *fp = nullptr)
    {
        int ret;
        this->fp = fp;
        ret = pthread_create(&t, NULL, [](void *arg) -> void *
                             {
    ...
    static void pstack()
    {
        ...
        char **symbols = backtrace_symbols(buffer, num_frames); // 将地址转换为符号
