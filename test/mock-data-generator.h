 ...
 * @return 返回包含所有进程数据的vector，每个元素是一个map，key为数据类型，value为对应的结构体数据
 */
inline std::vector<DKapture::DataHdr*> generate_mock_process_data(int num_processes = 3) {
    ...
    for (int i = 0; i < num_processes; ++i) {
        ...
        dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidTraffic));
        assert(dh);
        ...
        // 生成 ProcPidStatus 数据
        dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidStatus));
        assert(dh);
        ...
        for (int fd_idx = 0; fd_idx < 3; ++fd_idx) {
            dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidFd));
            assert(dh);
        ...
        dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidSchedstat));
        assert(dh);
        ...
        // 生成 ProcPidIo 数据
        dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidIo));
        assert(dh);
        ...
        // 生成 ProcPidStatm 数据
        dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidStatm));
        assert(dh);
        ...
        // 生成 ProcPidStat 数据
        dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidStat));
        assert(dh);
        ...
        for (int sock_idx = 0; sock_idx < 2; ++sock_idx) {
            dh = (DKapture::DataHdr *)calloc(1, sizeof(DKapture::DataHdr) + sizeof(ProcPidSock));
            assert(dh);
