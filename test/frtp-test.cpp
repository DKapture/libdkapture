...
TEST_F(FrtpBinaryTest, HelpOption)
{
	...
	// 验证输出包含关键信息
	EXPECT_NE(output_long.find("Usage:"), std::string::npos)
	...
	EXPECT_NE(output_long.find("--help"), std::string::npos)
	...
	// 验证退出码为0
	EXPECT_EQ(exit_code_short, 0)
		<< "Short help option should exit with code 0";

	// 验证短选项和长选项输出相同
	EXPECT_EQ(output_long, output_short)
...
TEST_F(FrtpBinaryTest, PolicyFileOption)
{
	...
	EXPECT_NE(exit_code, 0)
		<< "Expected non-zero exit code due to BPF permission issues";

	// 验证输出包含BPF相关的错误信息，表明程序尝试加载了策略文件
	EXPECT_NE(output.find("libbpf"), std::string::npos)
		<< "Output should contain libbpf error messages";
}
...
TEST_F(FrtpBinaryTest, InvalidOptions)
{
	...
	// 无效选项应该导致非零退出码
	EXPECT_NE(exit_code1, 0)
		<< "Invalid option should result in non-zero exit code";
	...
	// 无效选项应该导致非零退出码
	EXPECT_NE(exit_code2, 0)
		<< "Invalid short option should result in non-zero exit code";
	...
	// 缺少必需参数应该导致非零退出码
	EXPECT_NE(exit_code3, 0)
		<< "Missing required argument should result in non-zero exit code";
}
...
TEST_F(FrtpBinaryTest, ValidPolicyFile)
{
	...
	EXPECT_NE(output.find("libbpf"), std::string::npos)
		<< "Should reach BPF loading stage";
}
...
TEST_F(FrtpBinaryTest, NonexistentPolicyFile)
{
	...
	std::string output =
		runFrtpCommand({ "--policy-file", nonexistent_file });
	int exit_code = getLastExitCode();
...
TEST_F(FrtpBinaryTest, InvalidPolicyFile)
{
	...
	// 最终仍会因BPF权限而失败
	EXPECT_NE(exit_code, 0)
		<< "Should eventually fail due to BPF permissions";
}
...
TEST_F(FrtpBinaryTest, NoPermissionError)
{
	...
	EXPECT_EQ(exit_code, 255)
		<< "Should exit with code 255 due to BPF permission error";

	// 验证输出包含权限相关错误信息
	EXPECT_NE(output.find("RLIMIT_MEMLOCK"), std::string::npos)
		<< "Should mention RLIMIT_MEMLOCK";
...
TEST_F(FrtpBinaryTest, ExitCodes)
{
	...
	EXPECT_NE(getLastExitCode(), 0)
		<< "Invalid option should exit with non-zero code";
	...
	createTestPolicyFile(policy_content);
	runFrtpCommand({ "--policy-file", TEST_POLICY_FILE });
	EXPECT_EQ(getLastExitCode(), 255)
		<< "BPF permission error should exit with code 255";

	// 测试默认运行（无参数）的退出码
	runFrtpCommand({});
	EXPECT_EQ(getLastExitCode(), 255)
		<< "Default run should fail with BPF permission error";
}
...
struct PolicyRule
{
	...
	PolicyRule(const std::string &t, const std::string &i,
		   const std::string &a, const std::string &tgt)
		: type(t)
		, identifier(i)
		, action(a)
		, target(tgt)
...
class FrtpExtendedTest : public FrtpBinaryTest
{
	...
	bool validateOutputFormat(const std::string &output,
	...
	{
		return { "/tmp/test_file.txt", "/tmp/test_dir/nested_file.txt",
...
{
	std::vector<std::string> invalidTypes = { "process",	"process_id",
						  "executable", "binary",
						  "program",	"user",
						  "group" };
...
TEST_F(FrtpPolicyFormatTest, InvalidIdentifierFormats)
{
	...
	for (const auto &identifier : invalidProcIdentifiers)
	{
		...
		EXPECT_NE(exit_code, 0)
			<< "Invalid proc identifier should cause error: '"
			<< identifier << "'";
	}
	...
	for (const auto &identifier : invalidPidIdentifiers)
	{
		...
		EXPECT_NE(exit_code, 0)
			<< "Invalid PID identifier should cause error: '"
			<< identifier << "'";
	}
}
...
{
	std::vector<std::string> invalidActions = { "",	    "x",    "e",
						    "exec", "read", "write",
						    "rwe",  "wr",   "R",
						    "W",    "RW",   "rwx" };
	...
	for (const auto &action : invalidActions)
	{
		...
		EXPECT_NE(exit_code, 0) << "Invalid action '" << action
					<< "' should cause error";
	}
}
...
TEST_F(FrtpPolicyFormatTest, CommentHandling)
{
	std::string policy =
		...
		"## Double hash comment\n"
		"#forbid proc=/usr/bin/vim r /etc/hosts  # This should be ignored\n";
...
TEST_F(FrtpPathHandlingTest, AbsolutePaths)
{
	...
	for (const auto &path : absolutePaths)
	{
		...
		// 绝对路径应该被正确处理
		EXPECT_EQ(exit_code, 255)
			<< "Should handle absolute path: " << path;
	}
}
...
{
	std::vector<std::string> relativePaths = { "file.txt",
						   "./file.txt",
						   "../parent_file.txt",
						   "subdir/file.txt",
						   "./subdir/../file.txt",
...
TEST_F(FrtpPathHandlingTest, WildcardPaths)
{
	...
	std::vector<std::string> wildcardPaths = { "/tmp/*", "/usr/bin/*",
						   "/home/*", "/var/log/*",
						   "/etc/*" };
...
{
	std::vector<std::string> directoryPaths = { "/tmp/", "/etc/",
						    "/usr/bin/", "/var/log/",
						    "/home/user/" };
...
{
	std::vector<std::string> recursivePaths = { "/tmp/*", "/etc/*",
						    "/usr/bin/*", "/var/log/*",
						    "/home/user/*" };
...
TEST_F(FrtpPathHandlingTest, SpecialCharacterPaths)
{
	std::vector<std::string> specialPaths = {
		"/tmp/file with spaces.txt",	  "/tmp/file-with-dashes.txt",
		"/tmp/file_with_underscores.txt", "/tmp/file.with.dots.txt",
		"/tmp/file@with@at.txt",	  "/tmp/file#with#hash.txt",
...
TEST_F(FrtpPathHandlingTest, LongPaths)
{
	...
	for (const auto &path : longPaths)
	{
		...
		// 长路径可能因为不存在而导致错误
		EXPECT_NE(exit_code, 0)
			<< "Long path might cause error due to non-existence: "
			<< path.length() << " chars";
	}
}
...
TEST_F(FrtpPathHandlingTest, NonexistentPaths)
{
	std::vector<std::string> nonexistentPaths = {
		"/nonexistent/path/file.txt", "/tmp/does_not_exist.txt",
		"/var/fake_directory/file.log", "/home/fake_user/document.txt",
	...
	for (const auto &path : nonexistentPaths)
	{
		...
		EXPECT_GT(errors.size(), 0)
			<< "Should produce error messages for nonexistent path: "
			<< path;
	}
}
...
TEST_F(FrtpPathHandlingTest, SymbolicLinks)
{
	...
	std::string policy = generatePolicyRule(rule.type, rule.identifier,
						rule.action, rule.target);
	createTestPolicyFile(policy);
...
TEST_F(FrtpErrorHandlingTest, InvalidCharacters)
{
	...
	for (const auto &rule : rulesWithInvalidChars)
	{
		...
		// 无效字符可能导致解析错误
		EXPECT_NE(exit_code, 0)
			<< "Invalid characters should cause parsing issues";
	}
}
...
TEST_F(FrtpOutputValidationTest, HelpMessageFormat)
{
	...
	// 验证必需的帮助信息组件
	std::vector<std::string> requiredElements = { "Usage:",
...
TEST_F(FrtpOutputValidationTest, ErrorMessageFormats)
{
	...
	EXPECT_GT(errors1.size(), 0)
		<< "Should produce error messages for invalid option";

	// 测试不存在文件的错误格式
	std::string output2 =
		runFrtpCommand({ "--policy-file", "/nonexistent/file.pol" });
	std::vector<std::string> errors2 = extractErrorMessages(output2);
	EXPECT_GT(errors2.size(), 0)
		<< "Should produce error messages for nonexistent file";
	...
	// BPF错误应该包含特定关键字
	std::vector<std::string> bpfKeywords = { "libbpf", "BPF",
						 "RLIMIT_MEMLOCK",
...
{
	// 创建复杂的策略文件
	std::string complexPolicy =
		"# Complex policy for verbose output testing\n"
		"forbid proc=/usr/bin/cat r /etc/passwd\n"
	...
	EXPECT_TRUE(hasProcessingInfo)
		<< "Should contain policy processing information";
}
...
TEST_F(FrtpOutputValidationTest, LogMessageFormats)
{
	...
	EXPECT_TRUE(hasProperLogFormat)
		<< "Should have properly formatted log messages";
}
...
TEST_F(FrtpOutputValidationTest, BPFErrorMessages)
{
	...
	std::vector<std::string> expectedBpfErrors = {
		"Failed to bump RLIMIT_MEMLOCK", "Operation not permitted",
		"failed to load object", "failed to load BPF skeleton"
	};
	...
	EXPECT_TRUE(hasExpectedErrors)
		<< "Should contain expected BPF error messages";
}
...
TEST_F(FrtpBoundaryTest, MaximumPolicyRules)
{
	...
	// 大量规则可能导致内存或解析问题
	EXPECT_NE(exit_code, 0)
		<< "Maximum rules might cause memory or parsing issues";
}
