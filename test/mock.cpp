...
int bpf_ringbuffer_push(RingBuffer *m_bpf_rb, int bpf_idx, DKapture::DataHdr *dh)
{
    int ret = 0;
    ...
    if (ret < 0)
        return -1;
    return write(m_bpf_rb->map_fd, dh,
        sizeof(DKapture::DataHdr) + dh->dsz);
}
    ...
    int __flags, int __fd, __off_t __offset)
{
    ...
    if (__addr == NULL && __len > page_size &&
        __offset == page_size && __prot == PROT_READ &&
        __flags == MAP_SHARED && data_area_sz >= page_size &&
        ((data_area_sz & (data_area_sz - 1)) == 0))
    {
        // 第一次映射只是为了获取一个足够大的、未使用的地址空间
        vaddr = (void *)syscall(__NR_mmap, __addr,
        ...
        char *p = (char *)vaddr;
        vaddr = (void *)syscall(__NR_mmap, p,
            page_size, __prot, __flags, __fd, __offset);
        if (vaddr == MAP_FAILED)
            return MAP_FAILED;
        p += page_size;
        __offset += page_size;
        ...
        p = (char *)syscall(__NR_mmap, p,
            data_area_sz, __prot, __flags, __fd, __offset);
        if (p == MAP_FAILED)
            return MAP_FAILED;
    ...
    return (void *)syscall(__NR_mmap, __addr,
        __len, __prot, __flags, __fd, __offset);
}
