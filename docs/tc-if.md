...
```
Interface Traffic: [MATCH]
  Ethernet Header:
    Source MAC: 52:54:00:59:c1:be
    Dest MAC:   52:54:00:c3:dd:72
    EtherType:  0x800 (IPv4)
    Packet Size: 66 bytes
  Packet Type Statistics:
    Type: IPv4 (ID: 1)
    Current Flow Rate: 23 Mbps
    Smooth Flow Rate: 23 Mbps (EMA)

Interface Traffic: [PASS] 178 bytes, 1 packets
  Ethernet Header:
    Source MAC: 52:54:00:c3:dd:72
    Dest MAC:   52:54:00:59:c1:be
    EtherType:  0x800 (IPv4)
...
Interface Traffic: [DROP] 492 bytes, 1 packets
  Ethernet Header:
    Source MAC: 52:54:00:c3:dd:72
    Dest MAC:   52:54:00:59:c1:be
    EtherType:  0x800 (IPv4)
...
根据实际输出，tc-if 会显示以下三种事件类型：

- **`[MATCH]`**：数据包匹配到规则但未进行速率限制（通常用于监控模式）
- **`[PASS]`**：数据包通过速率限制检查，正常转发
- **`[DROP]`**：数据包因超出速率限制而被丢弃

每个事件都包含完整的以太网头部信息、数据包大小、协议类型统计和当前流量速率信息。
